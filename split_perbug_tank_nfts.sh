...
# Usage:
#   IMAGE_NAME="/full/path/to/tank_sheet.png" ~/split_perbug_tank_nfts.sh
#   IMAGE_NAME="/full/path/to/tank_sheet.png" OUTPUT_DIR="$HOME/perbug-tank-nfts" UPSCALE_SIZE=512 ~/split_perbug_tank_nfts.sh
#
# Required:
#   IMAGE_NAME -> full path to the tank class sheet
#
# Optional:
#   OUTPUT_DIR   -> output folder (default: $HOME/perbug-tank-nfts)
...
OUTPUT_DIR="${OUTPUT_DIR:-$HOME/perbug-tank-nfts}"
...
log "Splitting Perbug tank NFT sheet from: $IMAGE_NAME"
...
if cell_w <= 0 or cell_h <= 0:
    raise SystemExit("[ERROR] Invalid image size for 16x16 grid.")

# Tank-style naming pools
COLOR_WORDS = {
    "red":    ["ember", "crimson", "infernal", "scarlet", "pyre"],
    "orange": ["cinder", "molten", "auric", "flare", "sunfire"],
    "yellow": ["golden", "solar", "gilded", "spark", "radiant"],
    "green":  ["verdant", "moss", "thorn", "viridian", "bog"],
    "cyan":   ["frost", "tidal", "mist", "glacier", "aqua"],
    "blue":   ["azure", "storm", "cobalt", "sapphire", "deep"],
    "purple": ["void", "amethyst", "dusk", "arcane", "violet"],
    "dark":   ["obsidian", "night", "onyx", "shadow", "basalt"],
    "white":  ["ivory", "ashen", "pearl", "moon", "pale"],
}
...
def flood_remove_background(rgba_img):
    """
    Remove light background by flood-filling from borders only.
    This preserves bright armor/glow inside the sprite better than blanket keying.
    """
    im = rgba_img.copy()
    px = im.load()
    w, h = im.size
    ...
    def near_bg(r, g, b):
        # tuned for pale/white generated background
...
def analyze_sprite(im):
    ...
    score += min(1.0, glow_ratio * 2.2) * 0.20
    score += min(1.0, fill_ratio * 1.5) * 0.15

    if color_family in ("purple", "cyan", "blue"):
        score += 0.08
    if color_family in ("orange", "red") and glow_ratio > 0.14:
        score += 0.07
    if fill_ratio > 0.42:
        score += 0.05
...
def build_name(index, color_family):
    ...
    right = TANK_WORDS[(index // len(COLOR_WORDS[color_family])) % len(TANK_WORDS)]
    return f"{left}_{right}"

metadata_path = os.path.join(output_dir, "perbug_tank_metadata.csv")
rows = []
...
print(f"[OK] Wrote {len(rows)} tank NFTs into {output_dir}")
