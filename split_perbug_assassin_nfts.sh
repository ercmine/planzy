...
# Usage:
#   IMAGE_NAME="/full/path/to/assassin_sheet.png" ~/split_perbug_assassin_nfts.sh
#   IMAGE_NAME="/full/path/to/assassin_sheet.png" OUTPUT_DIR="$HOME/perbug-assassin-nfts" UPSCALE_SIZE=512 ~/split_perbug_assassin_nfts.sh
#
# Required:
#   IMAGE_NAME -> full path to the assassin class sheet
#
# Optional:
#   OUTPUT_DIR   -> output folder (default: $HOME/perbug-assassin-nfts)
...
OUTPUT_DIR="${OUTPUT_DIR:-$HOME/perbug-assassin-nfts}"
...
log "Splitting Perbug assassin NFT sheet from: $IMAGE_NAME"
...
COLOR_WORDS = {
    "red":    ["crimson", "scarlet", "ember", "blood", "pyre"],
    "orange": ["cinder", "amber", "sunfire", "molten", "flare"],
    "yellow": ["golden", "solar", "spark", "gilded", "dawn"],
    "green":  ["verdant", "thorn", "moss", "viridian", "venom"],
    "cyan":   ["frost", "mist", "glacier", "aqua", "tidal"],
    "blue":   ["azure", "storm", "cobalt", "sapphire", "deep"],
    "purple": ["violet", "void", "dusk", "amethyst", "arcane"],
    "pink":   ["rose", "glimmer", "aurora", "blush", "prism"],
    "white":  ["ivory", "moon", "ashen", "pearl", "pale"],
    "dark":   ["shadow", "onyx", "obsidian", "night", "grave"],
}
...
def analyze_sprite(im):
    ...
    score += min(1.0, glow_ratio * 2.4) * 0.20
    score += min(1.0, fill_ratio * 1.10) * 0.08

    if color_family in ("purple", "dark", "blue", "red"):
        score += 0.12
    if glow_ratio > 0.16:
        score += 0.09
    if sat_avg > 0.52:
...
def build_name(index, color_family):
    ...
    right = ASSASSIN_WORDS[(index // len(COLOR_WORDS[color_family])) % len(ASSASSIN_WORDS)]
    return f"{left}_{right}"

metadata_path = os.path.join(output_dir, "perbug_assassin_metadata.csv")
rows = []
...
print(f"[OK] Wrote {len(rows)} assassin NFTs into {output_dir}")
