...
- Public endpoint: `https://geo.perbug.dev` via nginx reverse proxy + TLS.

## Critical checks
1. `curl -f https://geo.perbug.dev/health`
2. `curl -f https://geo.perbug.dev/ready`
3. `curl -f https://geo.perbug.dev/version`
4. `curl -f https://geo.perbug.dev/metrics`
