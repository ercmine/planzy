...
use dryad_tipping_program::{
    instruction::TippingInstruction, processor::Processor, state::TippingConfig,
};
...
fn initialize_tipping_config() {
    ...
    let data = to_vec(&TippingInstruction::InitializeConfig { admin, fee_bps: 250 }).unwrap();
    Processor::process(&program_id, &[payer_ai, config_ai.clone()], &data).unwrap();
    let config = TippingConfig::deserialize(&mut &config_ai.data.borrow()[..]).unwrap();
