...
use perbug_common::PerbugAmount;
...
pub fn run(cfg: &CliConfig, args: MintToArgs) -> Result<()> {
    ...
    let amount = PerbugAmount::from_display_str(&args.amount).context("invalid amount")?;
    println!("payer: {}", cfg.payer_pubkey());
    println!("mint: {mint}");
    println!("recipient owner: {recipient}");
    println!("amount: {} PERBUG ({} atomic units)", amount, amount.atomic());
    println!(
