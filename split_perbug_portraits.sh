...
# Usage:
#   IMAGE_NAME="/full/path/to/portrait_sheet.png" ~/split_perbug_portraits.sh
#   IMAGE_NAME="/full/path/to/portrait_sheet.png" OUTPUT_DIR="$HOME/perbug-portraits" ~/split_perbug_portraits.sh
#
# Required:
#   IMAGE_NAME  -> full input image path
#
# Optional:
#   OUTPUT_DIR  -> output folder (default: $HOME/perbug-portraits)
...
log "Splitting and naming portrait sheet from: $IMAGE_NAME"
...
names = [
    ...
    "portrait_scout.png",
    "portrait_caster.png",
    "portrait_support.png",
    "portrait_cleric.png",
    "portrait_assassin.png",
    "portrait_engineer.png",
...
# Background estimate from upper-left corner
bg = img.getpixel((5, 5))[:3]

def color_close(rgb, bg_rgb, threshold=18):
    return all(abs(int(rgb[i]) - int(bg_rgb[i])) <= threshold for i in range(3))

def key_background_to_alpha(cell, bg_rgb, threshold=18):
    out = cell.copy()
    px = out.load()
    cw, ch = out.size
    for y in range(ch):
...
for idx, out_name in enumerate(names):
    ...
    cell = key_background_to_alpha(cell, bg, threshold=18)
