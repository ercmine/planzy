...
export function computeCompletenessScore(place) {
    let score = 0;
...
function categoryScore(place, categoryToken) {
    if (!categoryToken)
        return 0.5;
    const normalized = categoryToken.toLowerCase();
    if (place.primaryCategory.toLowerCase() === normalized)
        return 1;
    if (place.secondaryCategories.map((item) => item.toLowerCase()).includes(normalized))
        return 0.88;
    return 0;
}
function contentRichnessScore(place) {
...
function engagementScore(place) {
    const reviews = clamp01(Math.log10(1 + (place.reviewCount ?? 0)) / 3);
...
export function scorePlaceForMode(input) {
    ...
    const score = (components.distance * weights.distance)
