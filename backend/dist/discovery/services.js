...
class SearchInfra {
    ...
    count(mode, results) {
        if (mode === "nearby")
            this.telemetry.nearbyQueries += 1;
        if (mode === "text")
            this.telemetry.textQueries += 1;
        if (mode === "category")
            this.telemetry.categoryQueries += 1;
        if (results === 0)
            this.telemetry.zeroResults += 1;
    }
    snapshot() {
...
function buildSearchCacheKey(mode, context) {
...
function assertSearchInputs(context, mode) {
    if (context.lat !== undefined && (context.lat < -90 || context.lat > 90))
        throw new Error("invalid_lat");
    if (context.lng !== undefined && (context.lng < -180 || context.lng > 180))
        throw new Error("invalid_lng");
    if (context.radiusMeters !== undefined && (context.radiusMeters < 100 || context.radiusMeters > 50_000))
        throw new Error("invalid_radius");
    if (mode === "nearby" && (context.lat === undefined || context.lng === undefined))
        throw new Error("nearby_requires_coordinates");
    if (mode === "text" && !context.query?.trim())
        throw new Error("text_search_requires_q");
    if (mode === "category" && !(context.categoryId || context.categorySlug))
        throw new Error("category_search_requires_category");
}
...
function buildRecommendationContext(context, userId) {
    return {
        userId,
        anonymous: !userId,
        city: context.city,
...
function mapResult(candidate, includeExplain) {
    return {
...
function paginate(items, pageSize, cursor) {
...
function applyBaseEligibility(place) {
    if (place.moderationState === "suppressed" || place.isClosed)
        return false;
    return true;
}
function recommendationLimitMultiplier(planTier) {
    if (planTier === PlanTier.ELITE)
        return 2.2;
    if (planTier === PlanTier.PLUS)
        return 1.5;
    return 1;
}
function applyBaseFilters(place, context) {
    if (!applyBaseEligibility(place))
        return false;
    if (context.filters?.openNow && place.openNow !== true)
        return false;
    if (context.filters?.minRating && (place.rating ?? 0) < context.filters.minRating)
        return false;
    if (context.filters?.hasPhotos && place.imageUrls.length === 0)
        return false;
    if (context.filters?.hasReviews && (place.reviewCount ?? 0) === 0)
        return false;
    if (context.filters?.priceLevels?.length && !context.filters.priceLevels.includes(place.priceLevel ?? -1))
        return false;
    if (context.filters?.priceLevelMax !== undefined && (place.priceLevel ?? Number.MAX_SAFE_INTEGER) > context.filters.priceLevelMax)
        return false;
    if (context.city && String(place.city ?? "").toLowerCase() !== context.city.toLowerCase())
        return false;
    if (context.region && !(place.neighborhood ?? "").toLowerCase().includes(context.region.toLowerCase()))
        return false;
    if (context.country && !place.sourceAttribution.join(" ").toLowerCase().includes(context.country.toLowerCase()))
        return false;
...
class CandidateGenerator {
    ...
    premiumExperience;
    resolver;
    constructor(repo, premiumExperience, resolver) {
        this.repo = repo;
        this.premiumExperience = premiumExperience;
    ...
    balance(candidates) {
        ...
        for (const candidate of candidates) {
            ...
            if ((categoryCount.get(category) ?? 0) >= maxPerCategory)
                continue;
            if (creator && (creatorCount.get(creator) ?? 0) >= maxPerCreator)
                continue;
            if (chain && (chainCount.get(chain) ?? 0) >= maxPerChain)
                continue;
...
async function rankPlaces(repo, context, mode, resolver) {
...
export class RecommendationService {
    ...
    balancer = new DiversityBalancer();
    constructor(repo, premiumExperience, resolver) {
        this.repo = repo;
        this.premiumExperience = premiumExperience;
        this.resolver = resolver;
    ...
    async getRecommendedPlacesForContext(userId, context) {
        return this.recommend(userId, context);
    }
    ...
    async getRelatedPlacesForPlace(userId, placeId, context) {
        ...
        const related = places
            ...
            if (overlap > 0)
                candidate.reasons = [...candidate.reasons, "related_category_overlap"];
            return candidate;
        })
    ...
    async getSuggestedCreators(userId, context) {
        const profile = userId ? await this.repo.getRecommendationProfile(userId) : undefined;
    ...
    async getSuggestedGuides(userId, context) {
        const profile = userId ? await this.repo.getRecommendationProfile(userId) : undefined;
...
export class DiscoveryFeedService {
    ...
    searchService;
    premiumExperience;
    adService = new AdInsertionService();
    constructor(recommendationService, nearbyService, trendingService, browseService, searchService, premiumExperience) {
    ...
    async feed(userId, mode, context) {
        let organicItems = [];
        if (mode === "for_you") {
        ...
        const organicCards = organicItems.map((item) => ({ type: "place", id: item.placeId, place: item }));
        const cards = this.adService.insertEveryTen(organicCards);
        const placeStreamItems = organicItems.map((item) => ({
...
export class CityPageService {
    ...
    recommendationService;
    browseService;
    constructor(trendingService, recommendationService, browseService) {
        this.trendingService = trendingService;
        this.recommendationService = recommendationService;
        this.browseService = browseService;
    }
    async getCityPage(userId, city) {
