...
const TRUST_LEVEL = { low: 1, developing: 2, trusted: 3, high: 4 };
export class CreatorGamificationService {
    config;
    profiles = new Map();
    publishDailyCounts = new Map();
    placeDailyCounts = new Map();
    lastPlacePublishDay = new Map();
    suppressionCounts = new Map();
    milestoneCompletionCounts = new Map();
    constructor(config = {}) {
        this.config = {
    ...
    featureShowcases(creatorId, showcaseIds) {
    ...
    getAdminSnapshot() {
        ...
        for (const [reason, count] of this.suppressionCounts.entries())
            suppressionCounts[reason] = count;
        const milestoneCompletionCounts = {};
        for (const [milestoneId, count] of this.milestoneCompletionCounts.entries())
            milestoneCompletionCounts[milestoneId] = count;
        return { config: this.config, suppressionCounts, milestoneCompletionCounts };
    }
    resolveSuppression(input, day, trustTier) {
        if (["hidden", "removed", "rejected"].includes(input.moderationState ?? "active"))
        ...
        if ((this.publishDailyCounts.get(dailyKey) ?? 0) >= this.config.maxQualifyingPublishesPerDay)
            return "daily_publish_cap";
        const placeDailyKey = `${input.creatorId}:${input.canonicalPlaceId}:${day}`;
        if ((this.placeDailyCounts.get(placeDailyKey) ?? 0) >= this.config.repeatedPlaceDailyCap)
            return "repeat_place_cap";
        const lastPlaceKey = `${input.creatorId}:${input.canonicalPlaceId}`;
        const lastDay = this.lastPlacePublishDay.get(lastPlaceKey);
        if (lastDay && this.daysBetween(lastDay, day) < this.config.repeatedPlaceCooldownDays)
            return "repeat_place_cooldown";
    ...
    applyQualifiedPublish(profile, input, day, week, occurredAt) {
        ...
        if (input.neighborhoodId)
            this.addCoverage(profile.coverageMaps.cityNeighborhoods, input.cityId, input.neighborhoodId);
        for (const categoryId of input.categoryIds)
            this.addCoverage(profile.coverageMaps.categoryPlaces, categoryId, input.canonicalPlaceId);
        if (input.districtId)
            this.addCoverage(profile.coverageMaps.districtPlaces, input.districtId, input.canonicalPlaceId);
        if (TRUST_LEVEL[profile.trustTier] >= TRUST_LEVEL.trusted && input.moderationState !== "pending_review") {
            ...
            if (!latest || this.daysBetween(latest, day) > 0)
                profile.cleanContributionDays += 1;
        }
        profile.lastUpdatedAt = occurredAt.toISOString();
    }
    ...
    updateShowcases(profile, occurredAt) {
        const unlocked = [];
        for (const definition of this.config.showcases) {
            ...
            if (hasAllMilestones && trustEligible) {
                const showcase = { showcaseId: definition.id, unlockedAt: occurredAt.toISOString(), featured: false };
    ...
    metricValue(profile, definition) {
    ...
    addCoverage(map, key, value) {
        const set = map.get(key) ?? new Set();
        set.add(value);
        map.set(key, set);
    }
    buildNotifications(unlockedMilestones, unlockedShowcases, suppressionReason) {
        const notifications = [];
        for (const milestone of unlockedMilestones)
            notifications.push(`milestone_unlocked:${milestone.id}`);
        for (const showcase of unlockedShowcases)
            notifications.push(`showcase_unlocked:${showcase.showcaseId}`);
        if (suppressionReason === "quality_gate")
            notifications.push("creator_tip:focus_on_quality");
        if (suppressionReason === "daily_publish_cap")
            notifications.push("creator_tip:post_tomorrow_for_streak");
    ...
    weeksBetween(startWeek, endWeek) {
    ...
    bump(previousDateOrWeek, currentDateOrWeek, currentCount, threshold) {
        if (!previousDateOrWeek)
            return 1;
        if (previousDateOrWeek === currentDateOrWeek)
            return currentCount;
