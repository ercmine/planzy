...
import { cloneItinerary } from "./store.js";
export class OutingPlannerService {
    deps;
    constructor(deps) {
        this.deps = deps;
    }
    async createOutingPlan(userId, request) {
    ...
    async listSavedItineraries(userId) {
        return this.deps.store.listSavedItinerariesByUser(userId);
    }
    async getSavedItinerary(userId, itineraryId) {
    ...
    async updateSavedItinerary(userId, itineraryId, patch) {
        const itinerary = await this.requireOwnedItinerary(userId, itineraryId);
        if (typeof patch.title === "string" && patch.title.trim())
            itinerary.title = patch.title.trim();
        if (typeof patch.favorite === "boolean")
            itinerary.favorite = patch.favorite;
        if (typeof patch.archived === "boolean")
            itinerary.archived = patch.archived;
        if (patch.visibility)
            itinerary.visibility = patch.visibility;
        await this.deps.store.updateSavedItinerary(itinerary);
        return itinerary;
    }
    async regenerateItinerary(userId, request) {
        const quota = await this.deps.access.checkQuotaAccess(this.target(userId), QUOTA_KEYS.AI_REQUESTS_PER_DAY, 1);
        if (!quota.allowed)
            return { error: "regeneration_limit_reached", access: quota };
        if (!request.itineraryId)
            throw new ValidationError(["itineraryId is required"]);
        const existing = await this.getSavedItinerary(userId, request.itineraryId);
        const active = existing.activeRevision?.generated;
        if (!active)
            throw new ValidationError(["active revision missing"]);
    ...
    async persistRevision(itinerary, generated, source) {
        const revisions = await this.deps.store.listRevisions(itinerary.id);
    ...
    async requireOwnedItinerary(userId, itineraryId) {
        const itinerary = await this.deps.store.getSavedItinerary(itineraryId);
        if (!itinerary || itinerary.ownerUserId !== userId)
            throw new ValidationError(["itinerary not found"]);
        return itinerary;
    }
    includePlace(place, request, followedCreatorIds) {
        if (place.moderationState === "suppressed" || place.isClosed)
            return false;
        if (request.city && String(place.city ?? "").toLowerCase() !== request.city.toLowerCase())
            return false;
        if (request.exclusions?.includes(place.primaryCategory.toLowerCase()))
            return false;
        if (request.categoryPreferences?.length && !request.categoryPreferences.includes(place.primaryCategory.toLowerCase()))
            return false;
        if (request.creatorOnly && (!place.creatorId || !followedCreatorIds.includes(place.creatorId)))
            return false;
        return true;
    }
    scorePlace(place, request, followedCreatorIds) {
        let score = place.qualityScore * 0.45 + place.trendingScore * 0.2 + place.popularityScore * 0.15;
        if (request.budgetLevel === "low" && (place.priceLevel ?? 2) <= 1)
            score += 0.15;
        if (request.budgetLevel === "high" && (place.priceLevel ?? 1) >= 3)
            score += 0.12;
        if (request.groupType === "family" && !["bar", "nightlife"].includes(place.primaryCategory.toLowerCase()))
            score += 0.1;
        if (place.openNow)
            score += 0.07;
        if (place.creatorId && followedCreatorIds.includes(place.creatorId))
            score += 0.08;
        return score;
    }
    sequenceStops(places, maxStops) {
        const deduped = new Set();
        const picks = [];
        const sorted = [...places].sort((a, b) => a.primaryCategory.localeCompare(b.primaryCategory));
        for (const place of sorted) {
            if (deduped.has(place.canonicalPlaceId))
                continue;
            if (picks.some((row) => row.primaryCategory === place.primaryCategory))
                continue;
    ...
    buildItinerary(places, request, planTier, followedCreatorIds, degradedMode) {
        if (places.length === 0) {
            return {
                id: `gen_${randomUUID()}`,
        ...
        const sourceAttributions = places.flatMap((place) => {
            const rows = [{ sourceType: "place", sourceId: place.canonicalPlaceId, label: place.name }];
            if (place.creatorId)
                rows.push({ sourceType: "creator", sourceId: place.creatorId, label: "creator source" });
            return rows;
        });
        return {
            id: `gen_${randomUUID()}`,
    ...
    async enforceQuota(userId, key) {
        const aiFeature = await this.deps.access.checkFeatureAccess(this.target(userId), FEATURE_KEYS.AI_TRIP_ASSISTANT);
        if (!aiFeature.allowed)
            throw new ValidationError(["AI itinerary generation is not enabled for this plan"]);
        const quota = await this.deps.access.checkQuotaAccess(this.target(userId), key, 1);
        if (!quota.allowed)
            throw new ValidationError(["Itinerary generation limit reached"]);
    }
}
