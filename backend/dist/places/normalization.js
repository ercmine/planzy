...
export function extractWebsiteDomain(value) {
...
export function normalizeAddressComparison(input) {
    return normalizeWhitespace(`${input.formattedAddress ?? ""} ${input.address1 ?? ""} ${input.locality ?? ""} ${input.region ?? ""} ${input.postalCode ?? ""}`)
        .toLowerCase()
        .replace(/[.,]/g, "")
        .replace(/\b(st|street)\b/g, "street")
        .replace(/\b(ave|avenue)\b/g, "avenue");
