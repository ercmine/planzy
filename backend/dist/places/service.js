...
export class PlaceNormalizationService {
    ...
    normalizeProviderPlace(provider, rawPayload, sourceUrl) {
        return getProviderAdapter(provider).normalizeProviderPlace(rawPayload, { sourceUrl });
    }
    findPlaceMatch(normalizedProviderPlace) {
    ...
    getCanonicalPlaceByProviderRef(provider, providerPlaceId) {
    ...
    listSourceRecordsForPlace(placeId) {
        return this.store.listSourceRecordsForPlace(placeId);
    }
    rebuildCanonicalPlace(placeId) {
    ...
    reprocessSourceRecord(sourceRecordId) {
