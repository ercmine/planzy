...
export class PlaceMaintenanceService {
    ...
    detectDuplicateCandidates() {
        ...
        for (let i = 0; i < places.length; i += 1) {
            for (let j = i + 1; j < places.length; j += 1) {
                ...
                const addressScore = similarity([a.formattedAddress, a.address1, a.locality, a.region].filter(Boolean).join(" "), [b.formattedAddress, b.address1, b.locality, b.region].filter(Boolean).join(" "));
                if (addressScore > 0.7) {
                    score += 0.2;
    ...
    mergePlaces(input) {
        ...
        if (uniqueSources.length === 0)
            throw new Error("missing_sources");
        ...
        for (const sourceId of uniqueSources) {
            ...
            if (source.status === "merged")
                throw new Error(`source_already_merged:${sourceId}`);
            const dist = distanceMeters(target, source);
            if (!input.allowFarDistance && dist > 2500)
                throw new Error(`merge_distance_too_far:${sourceId}:${dist.toFixed(0)}`);
            mergedTarget = {
                ...mergedTarget,
    ...
    reassignAttachment(input) {
        ...
        if (!link)
            throw new Error("attachment_link_not_found");
