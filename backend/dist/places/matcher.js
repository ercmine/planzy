...
function isChainLike(name) {
    return name
        .toLowerCase()
        .split(/\s+/)
        .some((token) => CHAIN_TOKENS.has(token));
}
function scoreCandidate(normalized, candidate) {
    const reasons = [];
    let score = 0;
    ...
    const distance = haversineMeters({ lat: normalized.latitude, lng: normalized.longitude }, { lat: candidate.latitude, lng: candidate.longitude });
    if (distance < 30) {
        score += 0.3;
...
export function findPlaceMatch(normalized, candidates) {
    let best = { outcome: "no_match", score: -Infinity, reasons: [] };
