...
export class AccomplishmentsService {
    ...
    analyticsService;
    notificationService;
    constructor(store, analyticsService, notificationService) {
        this.store = store;
    ...
    setFeaturedBadges(userId, badgeIds) {
        const state = this.store.getUserState(userId) ?? defaultState(userId);
    ...
    async recordEvent(event) {
        ...
        if (!accepted) {
            try {
                await this.analyticsService?.track({ type: "accomplishment_event_blocked", reason: "anti_spam_or_quality", eventType: event.type }, {});
            }
            catch { }
            this.store.saveUserState(state);
        ...
        for (const unlock of unlocks) {
            try {
                await this.analyticsService?.track({ type: "accomplishment_unlocked", definitionId: unlock.definitionId, kind: unlock.kind }, {});
            }
            catch { }
            try {
    ...
    applyEvent(state, event) {
        if (event.type === "review_created") {
            if (event.contributionState && event.contributionState !== "published")
                return false;
            if (event.canonicalPlaceId) {
                const key = `review:${event.canonicalPlaceId}`;
                if ((state.collectibleProgress["__reviewedPlaceIds"] ?? []).includes(key))
                    return false;
                state.collectibleProgress["__reviewedPlaceIds"] = [...(state.collectibleProgress["__reviewedPlaceIds"] ?? []), key];
            }
            state.stats.reviewsCount += 1;
            if (event.categoryId && !state.stats.distinctReviewedCategories.includes(event.categoryId))
                state.stats.distinctReviewedCategories.push(event.categoryId);
            if (event.cityId && !state.stats.exploredCityIds.includes(event.cityId))
                state.stats.exploredCityIds.push(event.cityId);
        }
        ...
        if (event.type === "place_saved") {
            if (!event.canonicalPlaceId)
                return false;
            if (state.stats.savedPlaceIds.includes(event.canonicalPlaceId))
                return false;
        ...
        if (event.type === "place_explored") {
            if (!event.cityId)
                return false;
            if (!state.stats.exploredCityIds.includes(event.cityId))
                state.stats.exploredCityIds.push(event.cityId);
        }
    ...
    updateCollectibles(state, event) {
        if (!event.canonicalPlaceId)
            return;
        for (const definition of this.store.listDefinitions()) {
            if (definition.kind !== "collectible" || !definition.collectible)
                continue;
            const c = definition.collectible;
            if (c.cityId && event.cityId !== c.cityId)
                continue;
    ...
    evaluateUnlocks(state) {
        ...
        for (const definition of this.store.listDefinitions()) {
            ...
            if (!definition.condition || state.earnedDefinitionIds.includes(definition.id))
                continue;
            if (!this.passesTrustGate(definition, state))
                continue;
    ...
    passesTrustGate(definition, state) {
        const gate = definition.trustGate;
        if (!gate)
            return true;
        if (gate.requireTrustedCreator && !state.stats.trustedCreator)
            return false;
        if ((gate.minTrustScore ?? 0) > state.stats.trustScore)
            return false;
        if (gate.maxModerationStrikes != null && state.stats.moderationStrikes > gate.maxModerationStrikes)
            return false;
        return true;
    }
