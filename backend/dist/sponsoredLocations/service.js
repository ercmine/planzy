...
function atomic(amount) { return BigInt(Math.round(amount * (10 ** PERBUG_DECIMALS))); }
...
export class SponsoredLocationsService {
    ...
    approvePlaceAccess(input) {
        ...
        if (!access)
            throw new ValidationError(["place access request not found"]);
    ...
    createCampaign(input) {
        if (new Date(input.endsAt) <= new Date(input.startsAt))
            throw new ValidationError(["campaign end must be after start"]);
        if (input.totalBudgetPerbug <= 0 || input.dailyBudgetPerbug <= 0)
            throw new ValidationError(["budgets must be positive"]);
        const campaign = {
            id: `camp_${randomUUID()}`,
            placeId: input.placeId,
            businessId: input.businessId,
    ...
    fundCampaign(input) {
        ...
        if (campaign.businessId !== input.businessId)
            throw new ValidationError(["campaign not owned by business"]);
        if (!["draft", "active", "paused"].includes(campaign.status))
            throw new ValidationError(["campaign cannot be funded in current status"]);
        ...
        const feeAtomic = (amountAtomic * BigInt(feeBps)) / 10000n;
        ...
        if (feeAtomic > 0n)
            this.addLedger({ campaign, type: "platform_fee", amountAtomic: feeAtomic, metadata: { feeBps } });
    ...
    startVisitSession(input) {
        ...
        if (campaign.status !== "active")
            throw new ValidationError(["campaign not active"]);
        const budget = this.requireBudget(campaign.id);
    ...
    verifyVisit(input) {
        ...
        if (placeCoordinate) {
            ...
            if (meterDistance > campaign.targetRadiusMeters)
                reasons.push("outside_geofence");
        }
        if (session.dwellSeconds < rule.dwellSeconds)
            reasons.push("insufficient_dwell_time");
        for (const action of rule.requiredActions) {
            if (!input.actions.includes(action))
                reasons.push(`missing_action_${action}`);
        }
        ...
        this.store.saveVisitSession(session);
        if (reasons.length > 0)
            this.flagFraud({ campaignId: campaign.id, placeId: campaign.placeId, userId: session.userId, visitSessionId: session.id, reason: reasons.join(","), severity: reasons.includes("impossible_travel") ? "high" : "medium" });
        return structuredClone(decision);
    }
    async claimReward(input) {
        const session = this.requireVisit(input.visitSessionId);
        if (session.userId !== input.userId)
            throw new ValidationError(["visit does not belong to user"]);
        const existing = this.store.getClaimByVisit(session.id);
        if (existing)
            return structuredClone(existing);
        const decision = this.store.getDecisionByVisit(session.id);
        if (!decision || !decision.eligible)
            throw new ValidationError(["visit is not eligible for reward"]);
        const campaign = this.requireCampaign(session.campaignId);
        const budget = this.requireBudget(campaign.id);
    ...
    setCampaignStatus(input) {
        const campaign = this.requireCampaign(input.campaignId);
        if (!input.adminOverride && input.businessId && campaign.businessId !== input.businessId)
            throw new ValidationError(["campaign ownership mismatch"]);
    ...
    issueRefund(input) {
        ...
        if (refundable <= 0n)
            throw new ValidationError(["no refundable balance"]);
    ...
    listBusinessCampaigns(businessId) { return this.store.listCampaignsByBusiness(businessId).map((campaign) => ({ campaign, budget: this.requireBudget(campaign.id), rule: this.requireRule(campaign.id), claims: this.store.listClaimsByCampaign(campaign.id) })); }
    listUserRewardHistory(userId) { return this.store.listClaimsByUser(userId); }
    listFraudFlags() { return this.store.listFraudFlags(); }
    listCampaignLedger(campaignId) { return this.store.listLedgerForCampaign(campaignId); }
    ...
    requireBudget(campaignId) {
        const budget = this.store.getBudget(campaignId);
        if (!budget)
            throw new ValidationError(["campaign budget not found"]);
        return budget;
    }
    requireRule(campaignId) {
        const rule = this.store.getRewardRule(campaignId);
        if (!rule)
            throw new ValidationError(["campaign reward rule not found"]);
        return rule;
    }
    requireVisit(visitSessionId) {
        const visit = this.store.getVisitSession(visitSessionId);
        if (!visit)
            throw new ValidationError(["visit session not found"]);
        return visit;
    }
    calculatePayout(campaign, rule) {
        if (rule.type === "decay_per_claim") {
            ...
            return (rule.payoutAtomic * (10000n - decayBps * BigInt(paidCount))) / 10000n;
        }
        if (rule.type === "first_x_daily") {
            ...
            if ((rule.firstXDaily ?? 0) > 0 && paidToday >= (rule.firstXDaily ?? 0))
                return 0n;
        }
