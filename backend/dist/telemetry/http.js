...
function canInspectTelemetry(req) {
...
export function createTelemetryHttpHandlers(service) {
    async function ingest(req, res, sessionId) {
        const body = await parseJsonBody(req);
        const events = Array.isArray(body)
            ? body
            : typeof body === "object" && body !== null && Array.isArray(body.events)
                ? body.events
                : null;
        if (!events) {
    ...
    async function aggregate(req, res, sessionId) {
        if (!canInspectTelemetry(req)) {
            sendJson(res, 401, { error: "Unauthorized" });
            return;
        }
