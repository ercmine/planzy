...
export class CreatorMonetizationService {
    ...
    accessEngine;
    tipAdapter;
    constructor(store, accounts, subscriptions, creatorStore, accessEngine, tipAdapter) {
        this.store = store;
    ...
    ensureProfileForCreator(creatorProfileId) {
    ...
    getProfile(creatorProfileId) {
        return this.ensureProfileForCreator(creatorProfileId);
    }
    evaluateEligibility(creatorProfileId) {
        const creator = this.creatorStore.getProfileById(creatorProfileId);
        if (!creator)
            return { eligible: false, status: "not_eligible", reasonCode: "creator_profile_missing", adminExplanation: "Creator profile does not exist", userExplanation: "Create a creator profile first." };
        if (creator.status === CreatorProfileStatus.SUSPENDED)
            return { eligible: false, status: "suspended", reasonCode: "moderation_suspended", adminExplanation: "Creator profile suspended.", userExplanation: "Monetization is disabled while your account is suspended." };
        if (creator.status !== CreatorProfileStatus.ACTIVE)
            return { eligible: false, status: "pending_review", reasonCode: "creator_not_active", adminExplanation: "Creator is not active.", userExplanation: "Monetization review requires an active creator profile." };
        ...
        if (!Boolean(ent.creator_monetization_tools))
            return { eligible: false, status: "not_eligible", reasonCode: "missing_plan_entitlement", adminExplanation: "Creator plan lacks monetization entitlements.", userExplanation: "Upgrade your creator plan to enable monetization tools." };
    ...
    getCapabilities(creatorProfileId) {
    ...
    updateSettings(actorUserId, creatorProfileId, patch) {
        const creator = this.creatorStore.getProfileById(creatorProfileId);
        if (!creator)
            throw new Error("CREATOR_NOT_FOUND");
        ...
        if (patch.tippingEnabled && !capabilities.canReceiveTips)
            throw new Error("CREATOR_TIPPING_NOT_ELIGIBLE");
        if (patch.premiumContentEnabled && !capabilities.canPublishPremiumContent)
            throw new Error("CREATOR_PREMIUM_NOT_ELIGIBLE");
        const updated = { ...profile, ...patch, updatedAt: new Date().toISOString() };
        this.store.saveProfile(updated);
        return updated;
    }
    adminUpdateStatus(adminUserId, creatorProfileId, input) {
        const profile = this.ensureProfileForCreator(creatorProfileId);
        const updated = {
            ...profile,
    ...
    async createTipIntent(senderUserId, input) {
        if (input.amountMinor < 100 || input.amountMinor > 250000)
            throw new ValidationError(["tip amount must be within configured range"]);
    ...
    listTipSummary(creatorProfileId) {
    ...
    setGuidePremiumMode(actorUserId, creatorProfileId, guideId, input) {
        const creator = this.creatorStore.getProfileById(creatorProfileId);
        if (!creator || creator.userId !== actorUserId)
            throw new Error("FORBIDDEN");
        const guide = this.creatorStore.getGuideById(guideId);
        if (!guide || guide.creatorProfileId !== creatorProfileId)
            throw new Error("GUIDE_NOT_FOUND");
        if (input.mode !== "free" && !this.getCapabilities(creatorProfileId).capabilities.canPublishPremiumContent)
            throw new Error("CREATOR_PREMIUM_NOT_ELIGIBLE");
        guide.monetization = {
            mode: input.mode,
    ...
    evaluateGuideAccess(viewerUserId, creatorProfileId, guideId) {
        const guide = this.creatorStore.getGuideById(guideId);
        if (!guide || guide.creatorProfileId !== creatorProfileId || !guide.monetization || guide.monetization.mode === "free")
            return { locked: false };
        const owner = this.creatorStore.getProfileById(creatorProfileId);
        if (viewerUserId && owner?.userId === viewerUserId)
            return { locked: false };
        if (!viewerUserId)
    ...
    createMembershipPlan(actorUserId, creatorProfileId, input) {
        const creator = this.creatorStore.getProfileById(creatorProfileId);
        if (!creator || creator.userId !== actorUserId)
            throw new Error("FORBIDDEN");
        if (!this.getCapabilities(creatorProfileId).capabilities.canOfferSubscriptionsFuture)
            throw new Error("CREATOR_MEMBERSHIP_NOT_ELIGIBLE");
        const now = new Date().toISOString();
        const plan = { id: `cmp_plan_${randomUUID()}`, creatorProfileId, code: input.code, status: "draft", monthlyPriceMinor: input.monthlyPriceMinor, currency: (input.currency ?? "USD").toUpperCase(), tierName: input.tierName, perks: input.perks ?? [], createdAt: now, updatedAt: now };
        this.store.createMembershipPlan(plan);
        return plan;
    }
    listAuditLogs(creatorProfileId) { return this.store.listAuditLogs(creatorProfileId); }
}
