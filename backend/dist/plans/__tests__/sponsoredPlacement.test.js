...
function makeOrganic(index, category = "food") {
    return {
        id: `organic-${index}`,
        source: "places",
...
function makeSponsored(index, category = "food") {
    return {
        id: `sponsored-${index}`,
        source: "promoted",
...
describe("applySponsoredPlacement", () => {
    it("caps sponsored insertion and spaces placements", () => {
        const result = applySponsoredPlacement(fixturePlans(), { input: baseInput }, { ratioN: 10, windowSize: 50, maxSponsoredTotal: 3, minGap: 10, includeDebug: true });
    ...
    it("labels inserted sponsored plans", () => {
        const result = applySponsoredPlacement(fixturePlans(), { input: baseInput }, { ratioN: 10, windowSize: 50, maxSponsoredTotal: 3, includeDebug: true });
        const insertedSponsored = result.plans.filter((plan) => plan.metadata?.sponsored === true);
    ...
    it("drops sponsored plans that do not match explicit single-category filter", () => {
        const result = applySponsoredPlacement(fixturePlans(), {
            input: {
                ...baseInput,
                categories: ["outdoors"]
            }
    ...
    it("avoids sponsored placement in first positions when enough organic plans exist", () => {
        const result = applySponsoredPlacement(fixturePlans(), { input: baseInput }, { placeFirstSponsoredAfter: 3, includeDebug: true });
        expect(result.plans.slice(0, 3).every((plan) => plan.source !== "promoted")).toBe(true);
    });
    it("is deterministic", () => {
        const first = applySponsoredPlacement(fixturePlans(), { input: baseInput }, { ratioN: 10, windowSize: 50, maxSponsoredTotal: 3 });
        const second = applySponsoredPlacement(fixturePlans(), { input: baseInput }, { ratioN: 10, windowSize: 50, maxSponsoredTotal: 3 });
