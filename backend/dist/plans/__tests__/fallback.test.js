...
function makePlan(id, source, category) {
    return {
        id,
        source,
        sourceId: id,
...
class FakeCuratedProvider {
    ...
    async searchPlans(input, _ctx) {
        this.calls += 1;
        this.categoriesSeen = input.categories;
        const plans = Array.from({ length: 30 }, (_, index) => makePlan(`curated-${index + 1}`, this.name, index % 2 === 0 ? "movies" : "food"));
        return { plans, source: this.name, nextCursor: null };
    }
}
