...
function makeSuccessFetch(counter) {
...
describe("PlacesProvider", () => {
    ...
    it("returns yelp results when google times out", async () => {
        ...
        const provider = new PlacesProvider({ googleApiKey: "g-key", yelpApiKey: "y-key", google: { timeoutMs: 10 } }, { fetchFn });
        const result = await provider.searchPlans(baseInput, { timeoutMs: 2_000 });
        expect(result.plans).toHaveLength(1);
    ...
    it("uses cache for repeated query within TTL", async () => {
        ...
        let idx = 0;
        const provider = new PlacesProvider({ googleApiKey: "g-key", yelpApiKey: "y-key", cacheTtlMs: 60_000 }, { fetchFn: makeSuccessFetch(counter), now: () => nowValues[Math.min(idx++, nowValues.length - 1)] });
