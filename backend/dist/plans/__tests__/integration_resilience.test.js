...
function makePlan(id, source, category = "food") {
    return {
        id,
        source,
        sourceId: id,
...
async function buildFallbackProviders() {
...
class FastProvider {
    ...
    async searchPlans() {
        const plans = Array.from({ length: this.count }, (_, index) => makePlan(`${this.name}-${index + 1}`, this.name, index % 2 === 0 ? "food" : "coffee"));
...
describe("ProviderRouter integration resilience", () => {
    it("handles timeout + partial success and still returns a usable deck", async () => {
        ...
        const providers = [new SlowProvider("p_slow", 500), new FastProvider("p_fast", 4), curated, byo];
    ...
    it("uses curated/byo fallback when all primary providers fail", async () => {
        const { curated, byo } = await buildFallbackProviders();
        const providers = [new ErrorProvider("p_err1"), new ErrorProvider("p_err2"), curated, byo];
        const router = new ProviderRouter({
            enforceNoScrape: false,
            providers,
            config: makeConfig(providers.map((provider) => provider.name)),
    ...
    it("gracefully degrades on quota denial", async () => {
        ...
        const providers = [pQuota, curated, byo];
    ...
    it("auto-disables unhealthy providers and continues with curated", async () => {
        ...
        const providers = [flaky, curated];
        const healthMonitor = new ProviderHealthMonitor({ maxConsecutiveFailures: 3, disableForMs: 60_000, halfOpenAfterMs: 10_000, minCallsForRate: 1000 }, { now: () => nowMs });
        const router = new ProviderRouter({
            enforceNoScrape: false,
            providers,
            config: makeConfig(providers.map((provider) => provider.name)),
