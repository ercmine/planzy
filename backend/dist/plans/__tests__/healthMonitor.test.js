...
describe("ProviderHealthMonitor", () => {
    it("disables provider on consecutive failures", () => {
        let now = 1_000;
        const monitor = new ProviderHealthMonitor({ maxConsecutiveFailures: 3, disableForMs: 60_000, minCallsForRate: 10 }, { now: () => now });
        for (let i = 0; i < 3; i += 1) {
    ...
    it("disables provider on low rolling success rate", () => {
        let now = 10_000;
        const monitor = new ProviderHealthMonitor({ windowSize: 10, minCallsForRate: 5, disableSuccessRate: 0.5, maxConsecutiveFailures: 10 }, { now: () => now });
    ...
    it("becomes eligible after disable window passes", () => {
        let now = 50_000;
        const monitor = new ProviderHealthMonitor({ maxConsecutiveFailures: 2, disableForMs: 1_000, halfOpenAfterMs: 200 }, { now: () => now });
    ...
    it("allows exactly one half-open probe", () => {
        let now = 100_000;
        const monitor = new ProviderHealthMonitor({ maxConsecutiveFailures: 2, disableForMs: 10_000, halfOpenAfterMs: 2_000 }, { now: () => now });
