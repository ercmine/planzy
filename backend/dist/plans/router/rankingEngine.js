...
function eventTimeScore(plan, now) {
...
function userIdeaBoost(plan, now) {
    const kind = typeof plan.metadata?.kind === "string" ? plan.metadata.kind : undefined;
...
function completenessScore(plan) {
    let score = 0;
...
function comparePlans(a, b, byId) {
...
export function rankPlansAdvanced(plans, ctx, opts) {
    ...
    if (opts?.includeScorecards) {
        const orderedScorecards = ranked.map((plan) => scoreById.get(plan.id)).filter((card) => card !== undefined);
