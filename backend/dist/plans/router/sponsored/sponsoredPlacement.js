...
export function isSponsoredPlan(plan, opts) {
...
export function ensureSponsoredLabel(plan, opts) {
...
function computeMaxRunSponsored(plans, opts) {
    let maxRun = 0;
    let run = 0;
...
function buildDebug(debug, includeDebug) {
    if (!includeDebug) {
        return undefined;
    }
    return debug;
}
export function applySponsoredPlacement(plans, ctx, opts) {
    ...
    const explicitCategory = resolvedOpts.respectExplicitCategoryFilter &&
        ctx.input.categories !== undefined &&
        ctx.input.categories.length === 1
        ? ctx.input.categories[0]
        : undefined;
    const categoryAllowedSponsored = [];
