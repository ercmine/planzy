...
function createDebug(headBefore, headAfter, applied, reason) {
    return {
...
function pickBucket(buckets, state, opts, relaxCap, relaxRun) {
    const activeBuckets = buckets.filter((bucket) => bucket.plans.length > 0).sort(compareBuckets);
    if (activeBuckets.length === 0) {
        return null;
    }
    const shouldPreferUnusedEarly = state.picks < EARLY_VARIETY_WINDOW &&
...
function shapeHead(head, maxRun, minCategories, categoryCapInWindow) {
    ...
    while (output.length < head.length) {
        const chosen = pickBucket(orderedBuckets, state, { maxRun, minCategories, categoryCapInWindow }, false, false) ??
...
export function applyColdStartBooster(plans, ctx, opts) {
