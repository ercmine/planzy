...
import { CURATED_TEMPLATES } from "./curatedData.js";
...
function mapsSearchLink(query, lat, lng) {
    const params = new URLSearchParams({ api: "1", query, center: `${lat},${lng}` });
    return `https://www.google.com/maps/search/?${params.toString()}`;
}
function balancedTemplates(templates, max) {
...
function selectTemplates(inputCategories, templates, max) {
    if (max <= 0) {
        return [];
    }
...
export class CuratedProvider {
    ...
    async searchPlans(input, ctx) {
        ...
        const templatePlans = this.includeTemplates
