...
function buildCacheKey(source, input) {
    return [
        source,
...
export class PlacesProvider {
    ...
    async searchPlans(input, ctx) {
        ...
        try {
            ...
            const sourceFailures = settled
                ...
                .filter((err) => err instanceof ProviderError);
    ...
    async fetchGoogle(input, ctx, errors) {
        ...
        try {
            ...
            const mapped = raw.map((item) => this.googleToPlan(item)).filter((plan) => plan !== null);
    ...
    async fetchYelp(input, ctx, errors) {
        ...
        const yelpPrice = input.priceLevelMax ? Math.max(1, Math.min(4, input.priceLevelMax)) : undefined;
        ...
        try {
            ...
            const mapped = raw.businesses.map((item) => this.yelpToPlan(item, input.openNow)).filter((plan) => plan !== null);
    ...
    yelpToPlan(business, openNowRequested) {
        ...
        const categories = business.categories?.map((cat) => cat.alias).filter((value) => typeof value === "string") ?? [];
        return {
            id: `yelp:${business.id}`,
            source: "yelp",
            ...
            photos: normalizeHttpUrl(business.image_url) ? [{ url: normalizeHttpUrl(business.image_url) }] : undefined,
