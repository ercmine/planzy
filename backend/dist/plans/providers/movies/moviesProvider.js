...
function deriveRegion(locale, fallback) {
    if (!locale) {
        return fallback;
    }
...
function buildMovieShowtimesLink(title, lat, lng) {
...
export class MoviesProvider {
    ...
    async searchPlans(input, ctx) {
        ...
        if (includeTheaters) {
            const theaterCacheKey = buildTheatersCacheKey(normalizedInput.location.lat, normalizedInput.location.lng, normalizedInput.radiusMeters, normalizedInput.openNow);
            ...
            if (cachedTheaters) {
                ...
                try {
                    ...
                    theatersPlans = validatePlanArray(found.map((plan) => ({
                        ...plan,
                        metadata: {
                            ...(plan.metadata ?? {}),
                            kind: plan.metadata?.kind ?? "theater"
                        }
                    })))
                        .sort((a, b) => {
    ...
    tmdbMovieToPlan(movie, lat, lng, includeMovieShowtimeLinks) {
