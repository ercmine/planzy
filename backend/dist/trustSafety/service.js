...
export class TrustSafetyService {
    ...
    getContentSummary(target, baseQuality = 0.5) {
        ...
        const riskPenalty = Math.max(aggregate.scoreSummary.spamScore, aggregate.scoreSummary.toxicityScore, aggregate.scoreSummary.duplicateScore, aggregate.scoreSummary.suspiciousActivityScore, aggregate.scoreSummary.fraudScore) * 0.45;
        ...
        const badges = [];
        if (trustScore >= 0.65 && aggregate.state === "active")
            badges.push("trusted_content");
        if (aggregate.state === "pending_review" || aggregate.state === "auto_limited")
            badges.push("under_review");
        if (["hidden", "removed", "rejected"].includes(aggregate.state))
            badges.push("not_public");
        return {
            target,
