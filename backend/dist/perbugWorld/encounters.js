export class PerbugEncounterRegistry {
    ...
    get(type) {
        ...
        if (!module)
            throw new Error(`encounter module missing for type ${type}`);
        return module;
    }
    has(type) {
...
export class NodeEncounterResolver {
    resolveType(context) {
        const { node, progression, squad } = context;
        if (node.nodeType === "boss")
            return { type: "boss", reason: "boss node requires boss encounter" };
        if (node.nodeType === "mission")
            return { type: "mission", reason: "mission node maps to mission encounter" };
        if (node.nodeType === "event")
            return { type: "event", reason: "event node maps to event encounter" };
        if (node.nodeType === "resource")
            return { type: "treasure", reason: "resource node yields treasure encounter" };
...
function buildReward(baseXp, perbug, resources, energy, unlocks) {
...
export function createDefaultEncounterRegistry() {
    ...
    const fallbackEvaluate = (resolution, successState) => ({
    ...
    const simpleModule = (type, title, description, rewardFactory) => ({
        type,
        buildPreview: (ctx) => ({
            title,
            description,
...
export function nodeTypeToEncounterType(nodeType) {
    switch (nodeType) {
...
export function computeSquadPower(context) {
    return context.squad.units.filter((u) => u.equipped).reduce((sum, u) => sum + u.power, 0);
}
export function defaultRewardPreview(context, type) {
    const base = 8 + context.node.difficulty * 2;
    if (type === "boss")
        return buildReward(base + 20, 4, { apex_core: 1 }, 2);
    if (type === "treasure")
        return buildReward(base, 1, { bio_dust: 2 + context.node.difficulty }, 1);
    if (type === "mission")
        return buildReward(base + 4, 2, { mission_data: 1 }, 1);
