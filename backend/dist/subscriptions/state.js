import { CancellationMode, ReasonCode, RenewalStatus, SubscriptionStatus } from "./types.js";
const ACTIVE_STATES = new Set([SubscriptionStatus.FREE, SubscriptionStatus.ACTIVE, SubscriptionStatus.TRIALING, SubscriptionStatus.CANCELED, SubscriptionStatus.PAST_DUE, SubscriptionStatus.GRACE_PERIOD]);
export function isScheduledForCancellation(subscription) {
...
export function accessEndsAt(subscription) {
    if (subscription.status === SubscriptionStatus.CANCELED)
        return subscription.cancelEffectiveAt ?? subscription.currentPeriodEndAt;
    if (subscription.status === SubscriptionStatus.TRIALING)
        return subscription.trialEndAt;
    if (subscription.status === SubscriptionStatus.GRACE_PERIOD)
        return subscription.graceEndAt;
...
export function resolveAccessWindow(subscription, now = new Date()) {
    const nowMs = now.getTime();
    const parse = (iso) => (iso ? new Date(iso).getTime() : undefined);
...
export function renewalBehaviorLabel(subscription) {
    if (subscription.renewalStatus !== RenewalStatus.UNKNOWN)
        return subscription.renewalStatus;
    if (isScheduledForCancellation(subscription) || !subscription.autoRenews)
        return RenewalStatus.AUTO_RENEW_OFF;
    return RenewalStatus.AUTO_RENEW_ON;
}
export function assertTransitionAllowed(previous, next) {
