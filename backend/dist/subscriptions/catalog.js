...
import { PlanInterval, PlanTier, SubscriptionTargetType } from "./types.js";
const defaults = Object.fromEntries(ENTITLEMENT_DEFINITIONS.map((d) => [d.key, d.defaultValue]));
function plan(input) {
    return {
        ...input,
...
export function getAvailablePlans(targetType) {
...
export function getFreePlan(targetType) {
    const free = getAvailablePlans(targetType).find((plan) => plan.tier === PlanTier.FREE);
    if (!free)
        throw new Error(`No free plan for ${targetType}`);
    return free;
}
