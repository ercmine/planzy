export class SocialGamificationService {
    ...
    async recordAction(event) {
        if (this.store.hasProcessedEvent(event.eventId))
            return { ignored: true, generatedMomentIds: [] };
        this.store.markProcessedEvent(event.eventId);
        if (event.suspicious)
            return this.block(event, "suspicious_ring_pattern");
        ...
        for (const instance of this.store.listChallengeInstances(event.actorUserId)) {
            ...
            if (outcome?.momentId)
                generatedMomentIds.push(outcome.momentId);
        }
        for (const goal of this.store.listGoalDefinitions()) {
            ...
            if (points <= 0)
                continue;
            if (goal.scope.cityId && goal.scope.cityId !== event.cityId)
                continue;
            if (goal.scope.categoryId && !(event.categoryIds ?? []).includes(goal.scope.categoryId))
                continue;
        ...
        await this.analytics?.track({ type: "social_gamification_action_recorded", eventType: event.type }, {});
        return { ignored: false, generatedMomentIds };
    }
    async applyToChallengeInstance(instance, event) {
        if (instance.status !== "active")
            return null;
        if (!instance.audience.participantUserIds.includes(event.actorUserId))
            return null;
        if (Date.parse(instance.endsAt) < Date.now())
            return null;
        ...
        if (completed) {
            ...
            await this.notifications?.notify({ type: "challenge.status.updated", recipientUserId: event.actorUserId, challengeId: instance.id, status: "completed" });
    ...
    pointsForRules(rules, event, currentPoints, distinctPlaceIds = []) {
        let gained = 0;
        for (const rule of rules) {
            ...
            if (rule.allowedContentStates?.length && !rule.allowedContentStates.includes(event.contentState)) {
    ...
    getCompetitionSummary(userId, cityId) {
        const privacy = this.getPrivacy(userId);
        if (!privacy.allowCompetition)
            return undefined;
    ...
    createMoment(userId, type, title, subtitle, metadata) {
