...
export class GamificationControlService {
    store;
    analyticsService;
    constructor(store, analyticsService) {
        this.store = store;
        this.analyticsService = analyticsService;
    }
    seedInitialRules(adminId) {
    ...
    updateDraft(ruleVersionId, adminId, updater) {
        const rule = this.store.getRuleVersion(ruleVersionId);
        if (!rule || rule.lifecycle !== "draft")
            throw new Error("Draft rule version not found");
    ...
    publishRuleVersion(ruleVersionId, adminId, effectiveFrom = nowIso()) {
        ...
        for (const rule of this.store.listRuleVersions()) {
            if (rule.lifecycle === "active")
                this.store.saveRuleVersion({ ...rule, lifecycle: "archived" });
        }
        const active = { ...target, lifecycle: "active", effectiveFrom, publishedAt: nowIso() };
        this.store.saveRuleVersion(active);
        this.logAdminAction(adminId, "publish_rules", { ruleVersionId, effectiveFrom });
        void this.analyticsService?.track({ type: "gamification_rule_published", ruleVersionId }, {});
        return active;
    }
    processEvent(event) {
        const activeRule = this.store.getActiveRuleVersion(nowIso());
        if (!activeRule)
            throw new Error("No active gamification rule version");
        ...
        if (!suppressed) {
            ...
            if (event.actionType === "place_reviewed")
                state.reviewCount += 1;
            if (event.actionType === "creator_video_published")
                state.videoCount += 1;
            if (event.trustScore >= 70)
                state.trustedActions += 1;
            for (const badge of activeRule.mechanics.badges) {
                if (state.badgeIds.includes(badge.id))
                    continue;
                if ((badge.trustGate ?? 0) > event.trustScore)
                    continue;
            ...
            for (const quest of activeRule.mechanics.quests) {
                if (state.completedQuestIds.includes(quest.id))
                    continue;
                if (event.actionType !== quest.actionType)
                    continue;
                if (event.occurredAt < quest.startAt || event.occurredAt > quest.endAt)
                    continue;
            ...
            for (const streakRule of activeRule.mechanics.streaks) {
                if (event.actionType !== streakRule.actionType)
                    continue;
                ...
                for (const milestone of streakRule.milestones) {
                    if (streak.currentDays === milestone)
                        streakMilestonesReached.push({ streakId: streakRule.id, days: milestone });
                }
            }
            for (const collection of activeRule.mechanics.collections) {
                if (!event.canonicalPlaceId)
                    continue;
                if (collection.cityId && collection.cityId !== event.cityId)
                    continue;
                if (collection.categoryId && collection.categoryId !== event.categoryId)
                    continue;
        ...
        void this.analyticsService?.track({ type: "gamification_decision", suppressed, reasons }, {});
        return decision;
    }
    recomputeUser(userId, adminId) {
    ...
    logAdminAction(actorId, action, metadata) {
        this.store.saveAdminAuditLog({ id: id("audit"), actorId, action, metadata, createdAt: nowIso() });
    }
    validateRule(rule) {
        if (rule.mechanics.xp.maxDailyXp < 0)
            throw new Error("maxDailyXp must be >= 0");
        if (rule.mechanics.xp.rules.length === 0)
            throw new Error("At least one XP rule is required");
    }
}
