...
const TERMINAL = ["approved", "rejected", "withdrawn", "expired", "revoked", "suspended"];
const URL_LINK_TYPES = new Set([
    "website",
    "instagram",
    "tiktok",
    "facebook",
...
export class VenueClaimsService {
    ...
    async getActiveOwnershipForBusinessActor(input) {
        const ownership = (await this.store.listOwnershipByPlace(input.placeId)).find((entry) => (entry.isActive
            && entry.verificationStatus === "verified"
            && entry.businessProfileId === input.businessProfileId));
        if (!ownership)
            throw new ValidationError(["active verified business claim required for this place"]);
        return ownership;
    }
    async assertCanManageClaimedPlace(placeId, actor) {
        ...
        if (!ownership && !actor?.isAdmin)
            throw new ValidationError(["user cannot manage this place"]);
        return ownership;
    }
    async createClaimDraft(input, actor) {
        const userId = this.requireUser(actor);
    ...
    async submitClaim(claimId, actor) {
        const userId = this.requireUser(actor);
        const claim = await this.store.getClaimById(claimId);
        if (!claim || claim.claimantUserId !== userId)
            throw new ValidationError(["claim not found"]);
        if (TERMINAL.includes(claim.status))
            throw new ValidationError(["claim can no longer be submitted"]);
    ...
    async getClaim(claimId, actor) {
        ...
        if (!claim)
            throw new ValidationError(["claim not found"]);
        if (!actor?.isAdmin && actor?.userId !== claim.claimantUserId)
            throw new ValidationError(["not authorized to view claim"]);
        return claim;
    }
    async addEvidence(claimId, input, actor) {
        const userId = this.requireUser(actor);
        const claim = await this.getClaim(claimId, actor);
        if (claim.claimantUserId !== userId)
            throw new ValidationError(["only claimant can add evidence"]);
    ...
    async listEvidence(claimId, actor) {
        await this.getClaim(claimId, actor);
        return this.store.listEvidence(claimId);
    }
    async reviewClaim(claimId, decision, reasonCode, notes, actor) {
        ...
        const status = existingPrimary ? "partially_approved" : "approved";
    ...
    async revokeOwnership(ownershipId, reasonCode, actor) {
        this.assertCanReview(actor);
        const own = await this.store.getOwnershipById(ownershipId);
        if (!own)
            throw new ValidationError(["ownership not found"]);
        const nowIso = this.now().toISOString();
    ...
    async updateOfficialDescription(placeId, content, actor) {
        ...
        if (!safe)
            throw new ValidationError(["description content is required"]);
        const nowIso = this.now().toISOString();
        const record = {
            id: randomUUID(),
            placeId,
    ...
    async submitCategorySuggestion(placeId, input, actor) {
        const ownership = await this.assertCanManageClaimedPlace(placeId, actor);
        if (!input.primaryCategoryId && (!input.secondaryCategoryIds || input.secondaryCategoryIds.length === 0))
            throw new ValidationError(["at least one category suggestion is required"]);
        const nowIso = this.now().toISOString();
        const record = {
            id: randomUUID(),
            placeId,
    ...
    async updateManagedHours(placeId, input, actor) {
        const ownership = await this.assertCanManageClaimedPlace(placeId, actor);
    ...
    async upsertBusinessLink(placeId, input, actor) {
        const ownership = await this.assertCanManageClaimedPlace(placeId, actor);
        const nowIso = this.now().toISOString();
    ...
    async upsertMenuServices(placeId, input, actor) {
        const ownership = await this.assertCanManageClaimedPlace(placeId, actor);
        const nowIso = this.now().toISOString();
        const externalUrl = input.externalUrl ? normalizeUrl(input.externalUrl) : undefined;
        if (input.externalUrl && !externalUrl)
            throw new ValidationError(["externalUrl must be a valid http/https url"]);
        const record = {
            id: randomUUID(),
            placeId,
    ...
    async upsertBusinessImage(placeId, input, actor) {
        const ownership = await this.assertCanManageClaimedPlace(placeId, actor);
        const nowIso = this.now().toISOString();
        const record = {
            id: randomUUID(),
    ...
    async upsertOfficialContent(placeId, ownershipId, contentType, value, actor) {
        const userId = this.requireUser(actor);
        const ownership = await this.store.getOwnershipById(ownershipId);
        if (!ownership || !ownership.isActive || ownership.placeId !== placeId)
            throw new ValidationError(["ownership not active"]);
        if (!actor?.isAdmin && ownership.primaryUserId !== userId)
            throw new ValidationError(["user cannot manage this place"]);
        const nowIso = this.now().toISOString();
        const record = {
            id: randomUUID(),
    ...
    async getPlaceManagementState(placeId, actor) {
    ...
    async buildPublicPlaceProjection(placeId, providerData) {
    ...
    async upsertBusinessContactMethod(placeId, input, actor) {
        await this.assertCanManageClaimedPlace(placeId, actor);
        return this.trustService.upsertContactMethod(placeId, input, actor);
    }
    async listBusinessContactMethods(placeId, actor) {
        await this.assertCanManageClaimedPlace(placeId, actor);
        return this.store.listBusinessContactMethods(placeId);
    }
    async verifyBusinessContactMethod(placeId, contactMethodId, input, actor) {
        await this.assertCanManageClaimedPlace(placeId, actor);
        if (!actor?.isAdmin && input.status !== "pending_verification")
            throw new ValidationError(["admin required for this verification action"]);
    ...
    async getBusinessTrustStatus(placeId, actor) {
        await this.assertCanManageClaimedPlace(placeId, actor);
    ...
    async getAdminBusinessTrustState(placeId, actor) {
        this.assertCanReview(actor);
        const publicView = await this.trustService.buildPublicTrustView(placeId);
        return {
            publicView,
    ...
    normalizeLinkByType(type, value) {
        if (type === "phone") {
            const normalizedPhone = normalizePhone(value);
            if (!normalizedPhone)
                throw new ValidationError(["phone link must be a valid phone number"]);
            return normalizedPhone;
        }
        if (type === "email") {
            const email = value.trim().toLowerCase();
            if (!/^[-a-z0-9._+]+@[-a-z0-9.]+\.[a-z]{2,}$/i.test(email))
                throw new ValidationError(["email link must be a valid email"]);
            return email;
        }
        if (URL_LINK_TYPES.has(type)) {
            const normalizedUrl = normalizeUrl(value);
            if (!normalizedUrl)
                throw new ValidationError([`${type} link must be a valid http/https url`]);
            return normalizedUrl;
        }
        throw new ValidationError(["unsupported link type"]);
    }
    inferVerificationLevel(evidence) {
        const set = new Set(evidence.map((e) => e.evidenceType));
        if (set.has("document") && set.has("email_domain"))
            return "high";
        if (set.has("document") || set.has("registration"))
            return "enhanced";
        if (set.size > 0)
            return "basic";
        return "none";
    }
    async track(placeId, eventType, actorUserId, metadata) {
    ...
    async setStatus(claimId, status) {
        const mapped = { pending: "under_review", verified: "approved", rejected: "rejected" };
