...
const CLAIM_TYPES = ["sole_owner", "manager_operator", "organization_representative", "franchise_representative"];
const OWNERSHIP_ROLES = ["owner", "manager", "operator", "org_admin", "franchise_manager"];
const CLAIM_STATUSES = ["draft", "submitted", "pending_verification", "under_review", "needs_more_info", "approved", "partially_approved", "rejected", "withdrawn", "expired", "suspended", "revoked"];
const EVIDENCE_TYPES = ["email_domain", "website_match", "phone", "document", "registration", "social_link", "source_consistency", "manual_note"];
function isObject(v) { return typeof v === "object" && v !== null; }
function str(v, key, details, max, req = false) {
    ...
    if (req && t.length === 0)
        details.push(`${key} cannot be empty`);
    return t || undefined;
}
...
export function validateBusinessClaimInput(x) {
    if (!isObject(x))
    ...
    if (claimType && !CLAIM_TYPES.includes(claimType))
        details.push(`claimType must be one of ${CLAIM_TYPES.join(", ")}`);
    if (requestedRole && !OWNERSHIP_ROLES.includes(requestedRole))
        details.push(`requestedRole must be one of ${OWNERSHIP_ROLES.join(", ")}`);
    ...
    const methods = Array.isArray(x.verificationMethodSelection)
        ? x.verificationMethodSelection.filter((v) => typeof v === "string" && EVIDENCE_TYPES.includes(v))
        : [];
    if (details.length)
...
export function validateListClaimsOptions(x) {
    ...
    const status = typeof x.status === "string" && CLAIM_STATUSES.includes(x.status) ? x.status : undefined;
