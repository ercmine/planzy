...
import { aggregateVerificationLevel, computeRankingBoost, deriveReviewerTrustStatus, evaluateReviewQuality } from "./trust.js";
...
function normalizeMediaOrdering(media) {
...
function decodeImageDimensions(buffer, mimeType) {
    ...
    if (mimeType === "image/jpeg") {
        if (buffer.length < 4 || buffer[0] !== 0xff || buffer[1] !== 0xd8)
            throw new ValidationError(["invalid jpeg data"]);
        let offset = 2;
...
export class MemoryReviewsStore {
    byId = new Map();
    helpfulVotes = new Map();
    reports = new Map();
    mediaUploadsById = new Map();
    mediaReports = [];
    trustProfiles = new Map();
    reviewTrustSignals = new Map();
    verificationEvidence = new Map();
    verificationSummaries = new Map();
    visitSessions = new Map();
    usedSourceIds = new Set();
    trustAuditLogs = new Map();
    businessResponsesById = new Map();
    businessResponseByReviewId = new Map();
    businessResponseRevisions = new Map();
    businessResponseModerationActions = new Map();
    businessResponseEvents = [];
    businessResponseNotifications = [];
    async listByPlace(input) {
        const sort = input.sort ?? "most_helpful";
        const cursor = decodeCursor(input.cursor);
        const limit = Math.max(1, Math.min(input.limit ?? 20, 50));
        const rows = [...this.byId.values()].filter((row) => {
            if (row.placeId !== input.placeId && row.canonicalPlaceId !== input.placeId)
                return false;
            if (input.includeHidden)
                return true;
            if (input.viewerUserId && row.authorUserId === input.viewerUserId)
                return row.deletedAt ? false : true;
        ...
        const filteredRows = rows.filter((row) => {
            ...
            if (input.trustedOnly)
                return trust?.reviewTrustDesignation === "trusted" || trust?.reviewTrustDesignation === "trusted_verified";
            if (input.verifiedOnly)
                return trust?.verificationLevel === "verified" || trust?.verificationLevel === "probable";
            return true;
        });
    ...
    async getById(reviewId, viewerUserId, includeHidden = false) {
        const row = this.byId.get(reviewId);
        if (!row)
            return null;
        if (!includeHidden && !PUBLIC_STATES.includes(row.moderationState) && row.authorUserId !== viewerUserId)
            return null;
        return this.toView(row, viewerUserId);
    }
    async getByPlaceAndAuthor(placeId, authorUserId) {
    ...
    async listByAuthorProfile(input) {
        const rows = [...this.byId.values()].filter((row) => {
            if (row.authorProfileType !== input.authorProfileType || row.authorProfileId !== input.authorProfileId)
                return false;
            if (input.viewerUserId && row.authorUserId === input.viewerUserId)
    ...
    async update(input) {
        ...
        if (!isOwner)
            throw new ValidationError(["cannot edit another user review"]);
    ...
    async softDelete(reviewId, actorUserId, now = new Date()) {
        const row = this.byId.get(reviewId);
        if (!row)
            throw new ValidationError(["review not found"]);
        if (row.authorUserId !== actorUserId)
            throw new ValidationError(["cannot delete another user review"]);
    ...
    async setModerationState(reviewId, state, reason, now = new Date()) {
        const row = this.byId.get(reviewId);
        if (!row)
            throw new ValidationError(["review not found"]);
    ...
    async voteHelpful(reviewId, userId) {
        ...
        if (row.authorUserId === userId)
            throw new ValidationError(["cannot vote on your own review"]);
        const votes = this.helpfulVotes.get(reviewId) ?? new Set();
        votes.add(userId);
    ...
    async unvoteHelpful(reviewId, userId) {
        const row = this.byId.get(reviewId);
        if (!row)
            throw new ValidationError(["review not found"]);
        const votes = this.helpfulVotes.get(reviewId) ?? new Set();
    ...
    async getReviewerTrustProfile(userId) {
        if (!this.trustProfiles.has(userId))
            this.recomputeTrustForUser(userId, new Date());
        return this.trustProfiles.get(userId);
    }
    async listTrustAuditLogs(userId) {
        return [...(this.trustAuditLogs.get(userId) ?? [])];
    }
    async applyTrustOverride(input) {
    ...
    async getReviewTrustSignals(reviewId) {
        return this.reviewTrustSignals.get(reviewId) ?? null;
    }
    async getReviewVerificationSummary(reviewId) {
        return this.verificationSummaries.get(reviewId) ?? null;
    }
    async listEligibleEvidenceForUser(input) {
    ...
    async applyReviewVerificationOverride(input) {
    ...
    async upsertVerificationEvidence(input) {
        const confidenceScore = Number(input.confidenceScore ?? input.evidenceStrength ?? 0);
        if (confidenceScore <= 0 || confidenceScore > 100)
            throw new ValidationError(["confidenceScore must be between 1 and 100"]);
        if (input.sourceId && this.usedSourceIds.has(input.sourceId))
            throw new ValidationError(["source evidence already used"]);
        ...
        if (input.sourceId)
            this.usedSourceIds.add(input.sourceId);
    ...
    async createMediaUpload(input) {
        ...
        if (!PUBLIC_MEDIA_MIME.has(input.mimeType))
            throw new ValidationError(["unsupported mime type"]);
        ...
        if (mediaType === "photo") {
            ...
            if (!buffer.byteLength)
                throw new ValidationError(["upload cannot be empty"]);
            ...
            if (!input.mimeType.startsWith("video/"))
                throw new ValidationError(["video mime type required"]);
            if (fileSizeBytes <= 0 || fileSizeBytes > MAX_VIDEO_BYTES)
                throw new ValidationError(["invalid video file size"]);
            if (durationMs != null && durationMs > MAX_VIDEO_DURATION_MS)
                throw new ValidationError(["video duration exceeds maximum"]);
            if (width <= 0 || height <= 0)
    ...
    async finalizeMediaUpload(input) {
        ...
        if (upload.ownerUserId !== input.ownerUserId)
            throw new ValidationError(["cannot finalize another user's upload"]);
        if (upload.status !== "pending")
            throw new ValidationError(["upload cannot be finalized"]);
        if (Date.parse(upload.expiresAt) < Date.now()) {
            upload.status = "expired";
            throw new ValidationError(["upload expired"]);
        }
        if (upload.mediaType !== "video")
            throw new ValidationError(["upload is already finalized"]);
        ...
        if (upload.fileSizeBytes <= 0 || upload.fileSizeBytes > MAX_VIDEO_BYTES)
            throw new ValidationError(["invalid video file size"]);
        if (upload.durationMs <= 0 || upload.durationMs > MAX_VIDEO_DURATION_MS)
            throw new ValidationError(["invalid video duration"]);
    ...
    async reportReviewMedia(mediaId, userId, reason) {
        const media = this.findMedia(mediaId);
        if (!media)
            throw new ValidationError(["review media not found"]);
    ...
    async setReviewMediaModerationState(reviewId, mediaId, state, reason, now = new Date()) {
        const row = this.byId.get(reviewId);
        if (!row)
            throw new ValidationError(["review not found"]);
    ...
    async deleteBusinessReply(input) {
        const row = this.byId.get(input.reviewId);
        if (!row?.businessReply || row.businessReply.id !== input.replyId)
            throw new ValidationError(["business reply not found"]);
        if (row.businessReply.businessProfileId !== input.businessProfileId)
            throw new ValidationError(["cannot delete another business reply"]);
    ...
    async createOrUpdateBusinessReviewResponse(input) {
        ...
        if (!row)
            throw new ValidationError(["review not found"]);
        if (row.placeId !== input.placeId && row.canonicalPlaceId !== input.placeId)
            throw new ValidationError(["review place mismatch"]);
        const nowIso = (input.now ?? new Date()).toISOString();
        ...
        const nextStatus = input.moderationRequired ? "pending" : "published";
        const nextModeration = input.moderationRequired ? "pending" : "approved";
        const nextVisibility = input.moderationRequired ? "draft" : "public";
        if (existing && existing.status !== "removed") {
            if (existing.businessProfileId !== input.businessProfileId)
                throw new ValidationError(["review already has active response from another business"]);
    ...
    async moderateBusinessReviewResponse(input) {
        ...
        if (!response)
            throw new ValidationError(["business response not found"]);
        const nowIso = (input.now ?? new Date()).toISOString();
    ...
    async removeOwnBusinessReviewResponse(input) {
        const response = this.businessResponsesById.get(input.responseId);
        if (!response)
            throw new ValidationError(["business response not found"]);
        if (response.businessProfileId !== input.businessProfileId)
            throw new ValidationError(["cannot delete another business response"]);
        const nowIso = (input.now ?? new Date()).toISOString();
        response.status = "removed";
        response.moderationStatus = "removed";
        response.visibilityStatus = "removed";
    ...
    async getBusinessReviewResponseByReview(reviewId, includeHidden = false) {
    ...
    async listBusinessReviewResponsesForModeration(input) {
    ...
    async listBusinessResponseEvents() {
        return [...this.businessResponseEvents];
    }
    async listBusinessResponseNotifications() {
        return [...this.businessResponseNotifications];
    }
    attachUploadsToReview(review, actorUserId, uploadIds, now, append = false) {
        ...
        const attached = uploadIds.map((uploadId, index) => {
            ...
            if (!upload)
                throw new ValidationError(["upload not found"]);
            if (upload.ownerUserId !== actorUserId)
                throw new ValidationError(["cannot attach another user's upload"]);
            if (upload.status !== "uploaded")
                throw new ValidationError(["upload must be finalized before attach"]);
            ...
            if (nextPhotos > MAX_PHOTOS_PER_REVIEW)
                throw new ValidationError(["too many photos attached to review"]);
            if (nextVideos > MAX_VIDEOS_PER_REVIEW)
                throw new ValidationError(["too many videos attached to review"]);
    ...
    pushRevision(responseId, content, revisionNumber, editedByUserId, moderationStatusSnapshot, createdAt) {
    ...
    pushModerationAction(responseId, actionType, actedByUserId, reasonCode, notes, createdAt) {
    ...
    pushEvent(eventType, response, actorUserId, createdAt) {
    ...
    toView(row, viewerUserId) {
        const votes = this.helpfulVotes.get(row.id) ?? new Set();
        row.helpfulCount = votes.size;
        const mediaVisible = row.media
            .filter((item) => {
            if (row.deletedAt || row.moderationState !== "published")
                return viewerUserId === row.authorUserId && item.uploadedByUserId === viewerUserId;
            if (viewerUserId === row.authorUserId)
                return item.moderationState !== "removed";
            if (item.mediaType === "video")
                return item.moderationState === "published" && item.processingState === "ready";
    ...
    recomputeTrustForReview(reviewId, now) {
        const review = this.byId.get(reviewId);
        if (!review)
            return;
        ...
        let verificationSummary = computedSummary;
        ...
        const reviewerStatus = reviewerProfile.manualOverrideStatus ?? reviewerProfile.trustStatus;
    ...
    recomputeTrustForUser(userId, now) {
        ...
        const qualitySignals = reviews.map((row) => this.reviewTrustSignals.get(row.id)).filter(Boolean);
        ...
        const trustScore = Math.round(Math.min(100, approvedReviewCount * 2.8
            + avgQuality * 0.25
            + originality * 0.15
            + helpfulTotal * 0.6
            + verifiedRatio * 20
        ...
        for (const review of reviews)
            this.recomputeTrustForReview(review.id, now);
    }
    createDefaultTrustProfile(userId, now) {
        const profile = {
            userId,
            trustStatus: "none",
            trustTier: "none",
