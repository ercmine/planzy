...
function calculateOriginalityScore(body) {
...
export function evaluateReviewQuality(input) {
    ...
    if (isTrustedEligible)
        reasons.push("trusted_quality_threshold_met");
    if (input.mediaCount > 0)
        reasons.push("has_media_evidence");
...
function toPublicLabel(level) {
    if (level === "verified")
        return "Verified Visit";
    if (level === "probable")
        return "Probable Visit";
    if (level === "weak")
        return "Visit signal";
    return "Not verified";
}
export function aggregateVerificationLevel(evidence, now = new Date()) {
    ...
    const verificationLevel = score >= 80
        ? "verified"
        : score >= 58
            ? "probable"
            : score >= 30
                ? "weak"
                : "none";
    if (suspiciousPenalty >= 40 && score < 40)
        reasonCodes.add("suspicious_signals_downgraded");
    return {
...
export function deriveReviewerTrustStatus(score, moderationHealthScore, suspiciousPenaltyScore) {
    if (moderationHealthScore < 35 || suspiciousPenaltyScore >= 50)
        return { trustStatus: "suspended", trustTier: "none" };
    if (score >= 85)
        return { trustStatus: "trusted", trustTier: "elite" };
    if (score >= 72)
        return { trustStatus: "trusted", trustTier: "core" };
    if (score >= 58)
        return { trustStatus: "candidate", trustTier: "emerging" };
    return { trustStatus: "none", trustTier: "none" };
}
export function computeRankingBoost(input) {
    if (input.moderationState !== "published")
        return -20;
    ...
    return Math.round(input.qualityScore * 0.35
