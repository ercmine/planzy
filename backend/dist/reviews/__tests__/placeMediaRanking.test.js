...
import { getPlaceCardMedia, getPlaceMediaGallery, getPlaceVideoShelf, rankPlaceMedia, scorePlaceMediaItem, selectPlaceHeroMedia } from "../placeMediaRanking.js";
...
describe("place media ranking", () => {
    it("penalizes tiny or unready quality media", () => {
        ...
        const weak = scorePlaceMediaItem(candidate({ id: "weak", quality: { width: 180, height: 120, hasThumbnail: false, processingState: "failed" } }), "place_detail_hero", true);
    ...
    it("gives recency advantages without only newest sort", () => {
        ...
        const oldVeryStrong = candidate({ id: "old", createdAt: "2023-02-10T00:00:00.000Z", quality: { width: 3000, height: 2000 }, trust: { isTrusted: true, isVerifiedVisit: true } });
    ...
    it("trust boosts can beat low quality spam", () => {
        const trusted = candidate({ id: "trusted", trust: { isTrusted: true, isVerifiedVisit: true, reviewTrustWeight: 20 }, author: { profileId: "a-trusted", profileType: "PERSONAL" } });
        const spam = candidate({ id: "spam", quality: { width: 640, height: 480 }, trust: { abusePenalty: 0.9 }, engagement: { views: 2000, helpfulVotes: 2, reports: 20, hides: 8 } });
    ...
    it("filters invalid media before ranking", () => {
        ...
        const removed = candidate({ id: "removed", moderation: { moderationState: "removed", removedAt: "2026-01-03T00:00:00.000Z" } });
        const hidden = candidate({ id: "hidden", moderation: { moderationState: "hidden" } });
    ...
    it("suppresses duplicates and limits uploader domination", () => {
        const media = [
            candidate({ id: "a1", author: { profileId: "u1", profileType: "PERSONAL" }, fingerprint: "dup" }),
            candidate({ id: "a2", author: { profileId: "u1", profileType: "PERSONAL" }, fingerprint: "dup" }),
            candidate({ id: "a3", author: { profileId: "u1", profileType: "PERSONAL" } }),
            candidate({ id: "b1", author: { profileId: "u2", profileType: "PERSONAL" }, trust: { isTrusted: true } }),
            candidate({ id: "c1", mediaType: "video", author: { profileId: "u3", profileType: "CREATOR" } }),
        ];
    ...
    it("supports hero, gallery, card, and video shelf helpers", () => {
        const media = [
            candidate({ id: "photo-hero", mediaType: "photo", trust: { isTrusted: true } }),
            candidate({ id: "video-1", mediaType: "video", sourceType: "review_creator", author: { profileId: "cr1", profileType: "CREATOR" } }),
            candidate({ id: "photo-2", mediaType: "photo", author: { profileId: "u2", profileType: "PERSONAL" } })
        ];
