...
describe("getPlaceReviewVideoSection", () => {
    it("filters by place and removes duplicate playback urls", async () => {
        ...
        const store = {
            listByPlace: async ({ placeId }) => ({ reviews: reviews.filter((item) => item.placeId === placeId) }),
        };
        ...
        const store = {
            listByPlace: async () => ({
                reviews: [
                    ...
                    makeReview({ id: "c", placeId: "p", authorProfileId: "c1", authorProfileType: "CREATOR", createdAt: "2025-01-01T00:00:00.000Z", trust: { reviewTrustDesignation: "trusted", verificationLevel: "verified", isVerifiedVisit: true, rankingBoostWeight: 20 } }),
                ]
            })
        };
