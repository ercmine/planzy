...
const permissiveObjectVerifier = {
    async verifyObjectExists() {
        return true;
    }
};
const defaultProcessingExecutor = {
    async process(input) {
        return {
...
const defaultModerationProvider = {
    async scan({ video }) {
        const corpus = [video.title, video.caption, video.sourceFileName].join(" ").toLowerCase();
        const scoreFor = (terms, weights) => Math.max(0, ...terms.map((term, idx) => corpus.includes(term) ? weights[idx] : 0));
        ...
        const decision = maxScore >= 0.93 ? "block" : maxScore >= 0.7 ? "review" : "safe";
        return {
            summary: {
...
export class VideoPlatformService {
    ...
    notificationService;
    moderationProvider;
    constructor(store, cfg, uploadVerifier = permissiveObjectVerifier, processingExecutor = defaultProcessingExecutor, moderationService, trustSafetyService, notificationService, moderationProvider = defaultModerationProvider) {
        this.store = store;
        this.cfg = cfg;
    ...
    async requestUploadSession(input) {
        const video = await this.requireOwnerVideo(input.userId, input.videoId);
        if (input.sizeBytes > this.cfg.maxUploadBytes)
            throw new Error("file_too_large");
        if (!["draft", "awaiting_upload", "failed_upload"].includes(video.status))
            throw new Error("video_not_ready_for_upload");
    ...
    async publish(input) {
        ...
        if (!readiness.ready)
            throw new Error(`video_not_publish_ready:${readiness.missing.join(",")}`);
        return this.completePublish(video);
    }
    async archiveVideo(input) {
        const video = await this.requireOwnerVideo(input.userId, input.videoId);
    ...
    async retryProcessing(input) {
        const video = await this.requireOwnerVideo(input.userId, input.videoId);
        if (video.status !== "failed_processing")
            throw new Error("video_not_failed_processing");
        return this.enqueueProcessing(video);
    }
    async retryUpload(input) {
        const video = await this.requireOwnerVideo(input.userId, input.videoId);
        if (video.status !== "failed_upload")
            throw new Error("video_not_failed_upload");
        await this.transition(video, "awaiting_upload");
        return video;
    }
    ...
    async updateDraft(input) {
        ...
        if (input.creatorWallet !== undefined)
            video.creatorWallet = input.creatorWallet;
        if (input.primaryTreeId !== undefined)
            video.primaryTreeId = input.primaryTreeId;
        if (input.tags !== undefined)
            video.tags = input.tags.slice(0, 20);
    ...
    sectionFor(video) {
        if (video.status === "archived")
            return "archived";
        if (["failed_upload", "failed_processing", "hidden", "rejected"].includes(video.status))
            return "needs_attention";
        if (["processing_queued", "processing", "moderation_pending", "publish_pending", "processed", "upload_received", "uploading"].includes(video.status))
            return "processing";
        if (video.status === "published")
            return "published";
        return "drafts";
    }
    computeStatusCounts(rows) {
        const counts = { drafts: 0, processing: 0, published: 0, needsAttention: 0, archived: 0 };
    ...
    async listStudio(userId, input) {
        ...
        const sorted = [...filtered].sort((a, b) => {
            if (input?.sort === "oldest")
                return a.lifecycle.updatedAt.localeCompare(b.lifecycle.updatedAt);
            if (input?.sort === "most_views")
        ...
        return sorted
            ...
            .map((video) => {
            const readiness = this.computePublishReadiness(video);
            const moderationTarget = { targetType: "place_review_video", targetId: video.id, placeId: video.canonicalPlaceId ?? "creator", subjectUserId: video.authorUserId };
    ...
    async getCreatorStudioAnalytics(userId) {
        const rows = await this.store.listByAuthor(userId);
    ...
    async recordVideoEvent(input) {
        const video = await this.store.getVideo(input.videoId);
        if (!video)
            throw new Error("video_not_found");
        const engagement = video.engagement ?? { views: 0, likes: 0, saves: 0, shares: 0, completionRate: 0 };
        if (input.event === "video_viewed")
            engagement.views += 1;
        if (input.event === "video_liked")
            engagement.likes += 1;
        if (input.event === "video_saved")
            engagement.saves += 1;
        if (input.userId && input.event === "video_liked")
            await this.store.likeVideo({ videoId: video.id, userId: input.userId, createdAt: new Date().toISOString() });
        if (input.userId && input.event === "video_saved")
            await this.store.saveVideo({ videoId: video.id, userId: input.userId, createdAt: new Date().toISOString() });
        if (input.event === "video_shared")
            engagement.shares += 1;
        if (input.event === "video_completed")
            engagement.completionRate = Math.min(1, engagement.completionRate + 0.05);
    ...
    async listSavedVideos(userId, input) {
        ...
        return { items: items.filter((row) => Boolean(row)), nextCursor: result.nextCursor };
    }
    async getReengagementSummary(userId, limit = 10) {
        ...
        for (const row of history.slice(0, limit)) {
            const video = await this.store.getVideo(row.videoId);
            if (video)
                recentVideos.push(await this.withViewerState(this.toFeedItem(video), userId));
        }
        const creatorAffinity = [...history.reduce((acc, row) => acc.set(row.creatorUserId, (acc.get(row.creatorUserId) ?? 0) + 1), new Map()).entries()]
            .map(([creatorUserId, watchEvents]) => ({ creatorUserId, watchEvents }))
            .sort((a, b) => b.watchEvents - a.watchEvents)
            .slice(0, 5);
        const placeAffinity = [...history.reduce((acc, row) => acc.set(row.canonicalPlaceId, (acc.get(row.canonicalPlaceId) ?? 0) + 1), new Map()).entries()]
            .map(([canonicalPlaceId, watchEvents]) => ({ canonicalPlaceId, watchEvents }))
            .sort((a, b) => b.watchEvents - a.watchEvents)
            .slice(0, 5);
        return { recentVideos, creatorAffinity, placeAffinity };
    }
    async reportVideo(input) {
        const video = await this.store.getVideo(input.videoId);
        if (!video)
            throw new Error("video_not_found");
        const target = { targetType: "place_review_video", targetId: video.id, placeId: video.canonicalPlaceId ?? "creator", subjectUserId: video.authorUserId };
    ...
    async listCreatorVideos(userId) {
    ...
    async getDiagnostics(now = new Date()) {
    ...
    computePublishReadiness(video) {
        const missing = [];
        if (!video.rawAssetKey)
            missing.push("raw_asset");
        if (!video.processedAssetKey)
            missing.push("processed_asset");
        if (!video.thumbnailAssetKey && !video.coverAssetKey)
            missing.push("thumbnail_or_cover");
        if (!video.title && !video.caption)
            missing.push("metadata");
        if (video.moderationStatus !== "approved")
            missing.push("moderation_approval");
        if (video.visibility === "private")
            missing.push("public_visibility");
        return { ready: missing.length === 0, missing };
    }
    async transition(video, target, options) {
        if (video.status === target && options?.allowNoop)
            return;
        const allowed = VALID_TRANSITIONS[video.status] ?? [];
        if (!allowed.includes(target))
            throw new Error(`invalid_video_transition:${video.status}->${target}`);
        video.status = target;
        video.lifecycle.updatedAt = new Date().toISOString();
        await this.store.updateVideo(video);
    }
    async failUpload(video, reason) {
    ...
    async failProcessing(video, reason) {
        video.failureReason = reason;
        video.lifecycle.failedAt = new Date().toISOString();
    ...
    async withViewerState(item, userId) {
        if (!userId)
            return item;
    ...
    placeSignalsFor(videos, placeId, context) {
        ...
        const placeTrust = this.trustSafetyService?.summarizePlace({
            placeId,
            contentTargets: placeVideos.map((row) => ({ targetType: "place_review_video", targetId: row.id, placeId: row.canonicalPlaceId, subjectUserId: row.authorUserId }))
        });
    ...
    creatorSignalsFor(videos, creatorId) {
        ...
        const creatorTrust = this.trustSafetyService?.summarizeCreator({
            ...
            contentTargets: creatorVideos.map((row) => ({ targetType: "place_review_video", targetId: row.id, placeId: row.canonicalPlaceId, subjectUserId: creatorId }))
        });
        return {
            creatorUserId: creatorId,
    ...
    isVisibleVideo(video) {
        if (!["published", "moderation_pending", "publish_pending"].includes(video.status))
            return false;
    ...
    signedUploadUrl(bucket, key, partNumber) {
    ...
    async requireOwnerVideo(userId, videoId) {
