...
export const FEED_SCOPE_PROFILES = {
    local: {
        localityWeight: 0.29,
...
export function rankPlaceLinkedVideoFeed(input) {
    ...
    const scored = candidates
        ...
        const rawScore = (components.locality * profile.localityWeight
...
function filterByScope(videos, scope, context, minCount) {
    ...
    const withinPrimary = nowVisible.filter((video) => {
        ...
        if (typeof distance === "number")
            return distance <= primaryRadius;
        return locationMatchesContext(video, context);
    });
    if (withinPrimary.length >= minCount)
        return withinPrimary;
    const withinWidened = nowVisible.filter((video) => {
        const distance = video.feedDebug?.distanceMeters;
        if (typeof distance === "number")
            return distance <= widenedRadius;
        return locationMatchesContext(video, context);
    });
    return withinWidened.length ? withinWidened : nowVisible;
}
function locationMatchesContext(video, context) {
    if (!context)
        return true;
    if (context.city && video.feedDebug?.placeCity && context.city.toLowerCase() === video.feedDebug.placeCity.toLowerCase())
        return true;
    if (context.region && video.feedDebug?.placeRegion && context.region.toLowerCase() === video.feedDebug.placeRegion.toLowerCase())
        return true;
...
function computeLocalityScore(input) {
    ...
    if (input.context?.city && input.place.city && input.context.city.toLowerCase() === input.place.city.toLowerCase())
        return 1;
    if (input.context?.region && input.place.region && input.context.region.toLowerCase() === input.place.region.toLowerCase())
        return 0.8;
    return 0.45;
}
function diversify(scored, diversityWeight) {
    const creatorSeen = new Map();
    const placeSeen = new Map();
    const ranked = [...scored];
...
function toFeedItem(candidate, scope) {
...
function computeFreshnessScore(ts) {
