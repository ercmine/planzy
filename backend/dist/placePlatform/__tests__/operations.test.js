...
describe("open-data operational infrastructure", () => {
    ...
    it("retries failed enrichment jobs and isolates failures", async () => {
        ...
        const observability = new OpenDataObservability();
        let first = true;
        const orchestrator = new OpenDataWorkerOrchestrator(queue, {
        ...
        }, { imports: 1, enrichment: 1, reindex: 1, backfills: 1, cache: 1 }, observability);
    ...
    it("uses rollout-safe discovery fallback when owned backend degrades", async () => {
        const owned = { nearby: async () => { throw new Error("down"); } };
        const fallback = { nearby: async () => [{ placeId: "p1", title: "Fallback" }] };
        const observability = new OpenDataObservability();
        const api = new RolloutSafeDiscoveryApi(owned, fallback, { useOwned: () => true }, new OpenDataCache({ discoveryTtlMs: 10_000, detailTtlMs: 10_000, geocodeTtlMs: 10_000 }, observability), observability);
    ...
    it("serves rollout-safe detail payload with fallback and cache", async () => {
        ...
        const observability = new OpenDataObservability();
        const api = new RolloutSafeDetailApi(owned, fallback, { useOwned: () => true }, new OpenDataCache({ discoveryTtlMs: 10_000, detailTtlMs: 10_000, geocodeTtlMs: 10_000 }, observability), observability);
