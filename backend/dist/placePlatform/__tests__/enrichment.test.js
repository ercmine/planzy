...
import { EnrichmentJobRunner, InMemoryEnrichmentRepository, InMemoryPlacePlatformRepository, normalizeGeoNamesResponse, normalizeOpenTripMapResponse, normalizeWikidataResponse, PlaceEnrichmentService, scoreCandidateMatch, selectPrioritizedPlaceImages } from "../index.js";
...
describe("place enrichment", () => {
    ...
    it("keeps OpenTripMap as fallback image when Wikidata image is missing", async () => {
        ...
        const metadata = placeRepo.getById(canonicalPlaceId)?.metadata;
        const primaryImage = metadata.primaryImage;
        const gallery = metadata.imageGallery;
        const attr = metadata.imageAttributionSummary;
    ...
    it("rejects weak fallback image matches and keeps no-image state", async () => {
        ...
        expect((placeRepo.getById(canonicalPlaceId)?.metadata).primaryImage).toBeUndefined();
    });
    ...
    it("merges Wikidata fields with attribution without changing canonical id", async () => {
        ...
        expect((placeRepo.getById(canonicalPlaceId)?.metadata.wikidata).image?.url).toContain('wikimedia');
    });
    it("applies GeoNames admin context safely", async () => {
        ...
        expect((place?.metadata.geonames).county).toBe("Paris");
    });
    it("uses OpenTripMap as supplemental source", async () => {
        const { service, placeRepo, canonicalPlaceId } = setup();
        await service.enrichPlace(canonicalPlaceId, "opentripmap");
        const place = placeRepo.getById(canonicalPlaceId);
        expect((place?.metadata.opentripmap).kinds).toContain("tourism");
    });
    ...
    it("handles upstream errors without corrupting place", async () => {
        const { placeRepo, canonicalPlaceId } = setup();
        const enrichmentRepo = new InMemoryEnrichmentRepository();
        const service = new PlaceEnrichmentService(placeRepo, placeRepo, placeRepo, enrichmentRepo, {
            wikidata: async () => {
