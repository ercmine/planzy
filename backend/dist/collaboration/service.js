...
import { BusinessMembershipRole, CreatorProfileStatus, ProfileType } from "../accounts/types.js";
import { ValidationError } from "../plans/errors.js";
import { QUOTA_KEYS } from "../subscriptions/accessEngine.js";
...
export class CollaborationService {
    ...
    notifications;
    now;
    constructor(store, accounts, claims, subscriptions, accessEngine, businessAnalytics, businessPremium, notifications, now = () => new Date()) {
        this.store = store;
    ...
    assertBusinessManager(actor) {
        if (actor.profileType !== ProfileType.BUSINESS)
            throw new ValidationError(["business context required"]);
    ...
    async assertBusinessCanManagePlaces(actor, businessProfileId, placeIds) {
        this.assertBusinessManager(actor);
        if (actor.profileId !== businessProfileId)
            throw new ValidationError(["business context mismatch"]);
        for (const placeId of placeIds) {
            await this.claims.getActiveOwnershipForBusinessActor({ placeId, businessProfileId, userId: actor.userId });
        }
    }
    ensureCreatorContext(actor, creatorProfileId) {
        if (actor.profileType !== ProfileType.CREATOR || actor.profileId !== creatorProfileId)
            throw new ValidationError(["creator context required"]);
    ...
    async createInvite(actor, input) {
        ...
        if (this.subscriptions && this.accessEngine) {
            ...
            if (!quota.allowed)
                throw new ValidationError(["invite limit reached for current plan"]);
        }
        const invite = { ...input, id: randomUUID(), createdByUserId: actor.userId, status: "invited", createdAt: nowIso, updatedAt: nowIso };
    ...
    async listBusinessInvites(actor, businessProfileId) { this.assertBusinessManager(actor); if (actor.profileId !== businessProfileId)
        throw new ValidationError(["business context mismatch"]); return this.store.listInvitesByBusiness(businessProfileId); }
    async listCreatorInvites(actor, creatorProfileId) { this.ensureCreatorContext(actor, creatorProfileId); return this.store.listInvitesByCreator(creatorProfileId); }
    async respondToInvite(actor, inviteId, decision, note) {
        ...
        if (invite.status !== "invited")
            throw new ValidationError(["invite is not actionable"]);
    ...
    async createCampaign(actor, input, placeIds) {
        ...
        const nowIso = this.now().toISOString();
        const campaign = { ...input, id: randomUUID(), createdByUserId: actor.userId, createdAt: nowIso, updatedAt: nowIso };
    ...
    async transitionCampaignStatus(actor, campaignId, nextStatus) {
        const campaign = await this.store.getCampaign(campaignId);
        if (!campaign)
            throw new ValidationError(["campaign not found"]);
        this.assertBusinessManager(actor);
        if (actor.profileId !== campaign.businessProfileId)
            throw new ValidationError(["business context mismatch"]);
        if (!CAMPAIGN_TRANSITIONS[campaign.status].includes(nextStatus))
            throw new ValidationError([`illegal status transition ${campaign.status} -> ${nextStatus}`]);
        const nowIso = this.now().toISOString();
    ...
    async linkContentToCampaign(actor, input) {
        const campaign = await this.store.getCampaign(input.campaignId);
        if (!campaign)
            throw new ValidationError(["campaign not found"]);
        this.ensureCreatorContext(actor, input.creatorProfileId);
        const nowIso = this.now().toISOString();
        const link = { ...input, id: randomUUID(), moderationStatus: "approved", createdAt: nowIso, updatedAt: nowIso };
    ...
    async addFeaturedPlacement(actor, input) {
        ...
        const nowIso = this.now().toISOString();
        const placement = { ...input, isActive: true, id: randomUUID(), createdAt: nowIso, updatedAt: nowIso };
    ...
    async listFeaturedForPlace(placeId) {
