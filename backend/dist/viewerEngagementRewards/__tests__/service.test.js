...
function makeService() {
    return new ViewerEngagementRewardsService(new MemoryViewerEngagementStore(), { getVideoContext: (videoId) => videoContext[videoId] ?? null });
}
describe("ViewerEngagementRewardsService", () => {
    ...
    it("daily caps work", () => {
        const service = makeService();
        const highRule = { ...service.listRules().find((r) => r.action === "watch"), baseRewardAtomic: 700000n, cooldownHours: 0, maxPerDay: 10 };
    ...
    it("sponsored reward pools decrement correctly", () => {
        ...
        const pool = service.createSponsoredPool({
            ...
            fundedAtomic: 1000000n,
            startsAt: new Date(Date.now() - 1000).toISOString(),
            endsAt: new Date(Date.now() + 60_000).toISOString(),
            active: true,
            eligibleActions: ["watch"],
            perUserDailyCapAtomic: 400000n
        });
