...
function normalizeHandle(input) {
    ...
    if (RESERVED_SLUGS.has(handle))
        throw new ValidationError(["handle is reserved"]);
    return handle;
}
...
function encodeFeedCursor(item) {
...
function normalizeSocialLinks(links, now) {
    if (links.length > 8)
...
function isPublicApprovedGuide(guide) {
    return guide.status === "published" && guide.visibility === "public" && guide.moderationStatus === "approved";
}
function normalizeGuidePlaceItems(guideId, items, now, max) {
    ...
    for (const item of next.sort((a, b) => a.sortOrder - b.sortOrder)) {
        ...
        if (!item.dayIndex && !item.timeBlock && seen.has(item.placeId))
            continue;
...
function normalizeGuideSections(guideId, sections) {
...
export class CreatorService {
    ...
    verificationService;
    notifications;
    constructor(store, accounts, reviews, subscriptions, accessEngine, monetizationGate, verificationService, notifications) {
        this.store = store;
        this.accounts = accounts;
        this.reviews = reviews;
        this.subscriptions = subscriptions;
        this.accessEngine = accessEngine;
        this.monetizationGate = monetizationGate;
        this.verificationService = verificationService;
        this.notifications = notifications;
    }
    async ensureCreatorFeature(userId, profileId, feature) {
    ...
    async createOrSyncCreatorProfile(userId, input) {
        ...
        if (!desiredSlug || RESERVED_SLUGS.has(desiredSlug))
            throw new ValidationError(["slug is reserved or invalid"]);
        ...
        if (desiredHandle) {
            ...
            if (existingHandle && existingHandle.id !== source.id)
    ...
    checkHandleAvailability(handle, currentProfileId) {
    ...
    async updateCreatorProfile(userId, profileId, input) {
        ...
        if (handle) {
            ...
            if (existingHandle && existingHandle.id !== existing.id)
                throw new Error("HANDLE_TAKEN");
        }
        const profile = {
            ...existing,
    ...
    async followCreator(userId, creatorProfileId) {
        const profile = this.store.getProfileById(creatorProfileId);
        if (!profile || !profile.isPublic || profile.status !== CreatorProfileStatus.ACTIVE)
            throw new Error("CREATOR_NOT_FOLLOWABLE");
        if (profile.userId === userId)
            throw new ValidationError(["cannot follow yourself"]);
        if (this.store.getFollow(creatorProfileId, userId))
    ...
    async unfollowCreator(userId, creatorProfileId) {
    ...
    listFollowedCreators(userId, limit = 200) {
        const follows = this.store
            ...
            .filter((row) => Boolean(row.follow && row.profile))
    ...
    async getFollowingFeed(userId, input) {
        ...
        const reviewItems = input?.type === "guides"
            ...
            : (await Promise.all(activeCreatorIds.map((creatorProfileId) => this.listPublicReviewsByCreator(creatorProfileId, "latest", 40, userId)))).flat()
                ...
                const itemType = media?.mediaType === "video" ? "video_review" : media?.mediaType === "photo" ? "photo_review" : "review";
                return {
        ...
        const all = [...reviewItems, ...guideItems]
            ...
            if (input?.type === "videos")
                return item.feedItemType === "video_review";
            if (input?.type === "reviews")
                return ["review", "photo_review", "video_review"].includes(item.feedItemType);
            if (input?.type === "guides")
                return item.feedItemType === "guide";
            return true;
        })
            .sort((a, b) => {
            if (b.surfacedAt !== a.surfacedAt)
    ...
    async getPlaceCreatorContent(placeId, viewerUserId, input) {
        const limit = Math.max(1, Math.min(input?.limit ?? 20, 50));
    ...
    async getPublicProfile(slug, viewerUserId, opts) {
    ...
    async listPublicReviewsByCreator(creatorProfileId, sort = "latest", limit = 20, viewerUserId) {
        if (!("listByAuthorProfile" in this.reviews))
            return [];
        const rows = await this.reviews.listByAuthorProfile({
    ...
    async createGuide(userId, creatorProfileId, input) {
        ...
        if (status === "published" && profile.status !== CreatorProfileStatus.ACTIVE)
            throw new Error("CREATOR_NOT_ACTIVE");
        ...
        if (this.accessEngine) {
            ...
            if (!quota.allowed)
                throw new Error("GUIDE_QUOTA_EXCEEDED");
        }
        const guideId = `cg_${randomUUID()}`;
        const placeItems = normalizeGuidePlaceItems(guideId, input.placeItems, now, 100);
        if (status === "published" && placeItems.length === 0)
            throw new ValidationError(["published guides must include at least one place"]);
    ...
    async updateGuide(userId, guideId, input) {
        ...
        if (nextStatus === "published" && placeItems.length === 0)
            throw new ValidationError(["published guides must include at least one place"]);
        const guide = {
            ...existing,
    ...
    getGuideBySlug(slug, guideSlug, viewerUserId) {
        const profile = this.store.getProfileBySlug(slug) ?? this.store.getProfileByHandle(slug.toLowerCase().replace(/^@+/, ""));
        if (!profile)
            throw new Error("CREATOR_NOT_FOUND");
    ...
    getCreatorAnalytics(userId, creatorProfileId) {
        const profile = this.ensureOwner(userId, creatorProfileId);
