...
import type { AffiliateConfig, AffiliateParams, DomainRule } from "./types.js";

function toSafeUrl(url: string): URL | null {
...
export function domainMatches(ruleDomain: string, actual: string): boolean {
...
export function appendParams(url: string, params: AffiliateParams): string {
  const parsed = toSafeUrl(url);
  if (!parsed) {
    return url;
  }
...
export function buildRedirectUrl(base: string, targetUrl: string, extraParams: AffiliateParams): string {
...
function resolveRule(rules: DomainRule[] | undefined, actualDomain: string): DomainRule | undefined {
  if (!rules?.length) {
    return undefined;
  }
...
export function wrapUrl(
  url: string,
  cfg: AffiliateConfig,
  ctx?: { sessionId?: string; planId?: string; linkType?: "booking" | "ticket" | "website" }
): string {
  if (!cfg.enabled) {
    return url;
  }
...
export function wrapPlanLinks(plan: Plan, cfg: AffiliateConfig, ctx?: { sessionId?: string }): Plan {
  if (!plan.deepLinks) {
    return plan;
  }
