...
function createRequest(path: string, headers?: Record<string, string>): IncomingMessage {
  return {
    method: "GET",
    url: path,
...
describe("createDeckHandler", () => {
  it("returns deck payload with mix counts", async () => {
    ...
    const fakeRouter: Pick<ProviderRouter, "search"> = {
      async search() {
        return {
          plans,
          nextCursor: "next-1",
    ...
    const handler = createDeckHandler({ router: fakeRouter as ProviderRouter });
    ...
    const body = JSON.parse(res.body ?? "{}") as {
      ...
      nextCursor: string;
      plans: Plan[];
      mix: { sponsoredCount: number; planSourceCounts: Record<string, number>; categoryCounts: Record<string, number> };
    };
  ...
  it("ignores unknown categories during parsing", async () => {
    let capturedCategories: string[] | undefined;
