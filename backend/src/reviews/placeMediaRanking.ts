...
interface ProfileWeights {
  ...
  freshness: number;
  engagement: number;
  diversityPenaltyPerPriorFromAuthor: number;
  videoPenalty: number;
  videoBoost: number;
}

interface RankingConfig {
  weights: Record<PlaceMediaSurface, ProfileWeights>;
  freshnessHalfLifeDays: Record<PlaceMediaSurface, number>;
  duplicatePenalty: number;
...
function qualityScore(item: PlaceMediaCandidate): number {
  if (item.quality.processingState && item.quality.processingState !== "ready") return 0;
...
function relevanceScore(item: PlaceMediaCandidate): number {
...
function trustScore(item: PlaceMediaCandidate): number {
...
function freshnessScore(item: PlaceMediaCandidate, surface: PlaceMediaSurface): number {
  const createdMs = Date.parse(item.createdAt);
  if (!Number.isFinite(createdMs)) return 0.3;
...
function engagementScore(item: PlaceMediaCandidate): number {
...
function duplicateKey(item: PlaceMediaCandidate): string {
...
function nearDuplicateKey(item: PlaceMediaCandidate): string {
...
function isDisplayable(item: PlaceMediaCandidate): boolean {
  if (item.moderation.isDeleted || item.moderation.isPrivate || item.moderation.legalBlocked) return false;
  if (item.moderation.removedAt) return false;
  if (item.moderation.moderationState && item.moderation.moderationState !== "published") return false;
  if (item.moderation.visibilityState && item.moderation.visibilityState !== "public") return false;
  if (item.quality.processingState && item.quality.processingState !== "ready") return false;
  if (item.mediaType === "video") {
    if (!item.playbackUrl && !item.quality.hasPlayableVideo) return false;
...
export function scorePlaceMediaItem(item: PlaceMediaCandidate, surface: PlaceMediaSurface, debug = false): RankedPlaceMediaItem {
  ...
  const breakdown: PlaceMediaScoreBreakdown | undefined = debug ? {
    total,
...
export function rankPlaceMedia(params: {
  ...
  const typeCounts = new Map<PlaceMediaType, number>();
  const diversified: RankedPlaceMediaItem[] = [];
  const pool = [...scored];
  while (pool.length > 0) {
    let bestIndex = 0;
...
export function selectPlaceHeroMedia(placeId: string, mediaItems: PlaceMediaCandidate[], debug = false): RankedPlaceMediaItem | undefined {
...
export function getPlaceMediaGallery(placeId: string, mediaItems: PlaceMediaCandidate[], limit = 24, debug = false): RankedPlaceMediaItem[] {
  ...
  if (!topVideo) return ranked.slice(0, limit);

  const result: RankedPlaceMediaItem[] = [];
  ...
  for (const entry of ranked) {
    ...
    if (consumed.has(entry.item.id)) continue;
    result.push(entry);
  }
...
export function getPlaceCardMedia(placeId: string, mediaItems: PlaceMediaCandidate[], debug = false): RankedPlaceMediaItem | undefined {
  return rankPlaceMedia({ placeId, mediaItems, surface: "search_result_card", limit: 1, preferredMediaType: "photo", debug })[0];
}

export function getPlaceVideoShelf(placeId: string, mediaItems: PlaceMediaCandidate[], limit = 12, debug = false): RankedPlaceMediaItem[] {
