...
import type { ChallengeDefinition, ChallengeEvent, ChallengeProgress, QuestHubResponse, UserChallengeState } from "./types.js";
...
export class ChallengesService {
  ...
  private isActive(definition: ChallengeDefinition, at = Date.now()): boolean {
  ...
  listAvailable(userId: string, filters?: { cityId?: string; neighborhoodId?: string; categoryId?: string; marketId?: string; track?: string; cadence?: string }) {
    const now = Date.now();
  ...
  getQuestHub(userId: string, filters?: { cityId?: string; marketId?: string; categoryId?: string; track?: string }): QuestHubResponse {
  ...
  getSummary(userId: string) {
    ...
    return {
      ...
      featuredLocales: challenges.map((item) => item.neighborhoodLabel ?? item.cityLabel ?? item.marketLabel).filter((item): item is string => Boolean(item)).slice(0, 4)
    };
  }
  ...
  async recordEvent(event: ChallengeEvent): Promise<{ ignored: boolean; completedChallengeIds: string[]; blockedReason?: string }> {
    ...
    for (const definition of this.store.listDefinitions()) {
      if (!this.isActive(definition)) continue;
      ...
      for (const criterion of definition.criteria) {
        ...
        if (!this.eventMatchesScope(event, criterion.scope ?? definition.scope)) continue;
  ...
  private eventMatchesScope(event: ChallengeEvent, scope?: { marketIds?: string[]; cityIds?: string[]; neighborhoodIds?: string[]; categoryIds?: string[]; hotspotIds?: string[]; canonicalPlaceIds?: string[] }): boolean {
    if (!scope) return true;
    if (scope.marketIds?.length && (!event.marketId || !scope.marketIds.includes(event.marketId))) return false;
  ...
  private async emitCompletionSignals(userId: string, definition: ChallengeDefinition) {
    try {
      await this.analyticsService?.track({ type: "challenge_completed", challengeId: definition.id, track: definition.track, scopeType: definition.scopeType, cadence: definition.cadence } as never, {} as never);
    } catch {}
