...
describe("initBackendGeoRuntime", () => {
  ...
  it("uses direct nominatim mode without probing upstream health", async () => {
    const fetchSpy = vi.fn<(input: string | URL | Request, init?: RequestInit) => Promise<Response>>(async () => new Response("not found", { status: 404 }));
  ...
  it("uses custom geo service mode and probes upstream /health", async () => {
    const fetchSpy = vi.fn<(input: string | URL | Request, init?: RequestInit) => Promise<Response>>(async () => new Response(JSON.stringify({ ok: true, mode: "remote", version: "1.0.0" }), {
      status: 200,
