export type WorldBiomeBandType =
...
const macroNames = ["Lumin Expanse", "Shatter March", "Verdant Circuit", "Signal Crown", "Glass Frontier"] as const;
const districtTypes: WorldDistrictType[] = [
  "urban_cluster",
  "village_ring",
  "woodland_corridor",
  "relic_basin",
...
const biomeBands: WorldBiomeBandType[] = [
  "enchanted_forest_belt",
  "crystal_plains",
  "marshy_lowlands",
  "ruin_laced_highlands",
...
const factionTypes: WorldFactionType[] = ["merchant_guild", "mission_control", "relic_hunters", "wild_hostiles", "anomaly_cabal"];

export function deriveWorldSeed(input: HierarchySeedInput): number {
  if (input.mode === "demo") return hash32("perbug-demo-world-v1");
  const quantized = `${Math.round(input.lat * 1000)}:${Math.round(input.lng * 1000)}`;
  return hash32(`perbug-real-world-v1:${quantized}`);
}

export function buildChunkRegionContext(lat: number, lng: number, seed: number): ChunkRegionContext {
  ...
  const macroRegion: WorldMacroRegion = {
    id: `macro_${macroCellX}_${macroCellY}_${macroIdx}`,
    name: macroNames[macroIdx],
    climateBias: pickFrom(["temperate", "humid", "arid", "storm"] as const, seed, macroCellX, macroCellY, "macro-climate"),
    mood: pickFrom(["settled", "frontier", "mystic", "contested"] as const, seed, macroCellX, macroCellY, "macro-mood"),
    terrainTint: pickFrom(["verdigris", "amber", "violet", "teal", "slate"] as const, seed, macroCellX, macroCellY, "macro-tint")
  };
  ...
  const biomeBand: WorldBiomeBand = {
  ...
  const factionZone: WorldFactionZone = {
  ...
  const progressionBand: WorldProgressionBand = progressionValue < 0.85
  ...
  const influenceField: RegionInfluenceField = {
...
function smoothNoise(seed: number, x: number, y: number, channel: string): number {
...
function valueNoise(seed: number, x: number, y: number, channel: string): number {
  const h = hash32(`${seed}:${channel}:${x}:${y}`);
  return (h >>> 0) / 0xffffffff;
}

function pickFrom<T>(arr: readonly T[], seed: number, x: number, y: number, channel: string): T {
  return arr[pickIndex(arr.length, seed, x, y, channel)];
}

function pickIndex(length: number, seed: number, x: number, y: number, channel: string): number {
  const h = hash32(`${seed}:${channel}:${x}:${y}`);
