...
import { NodeEncounterResolver, PerbugEncounterRegistry, computeSquadPower, createDefaultEncounterRegistry, defaultRewardPreview } from "./encounters.js";
import type {
  EncounterAnalyticsEvent,
  EncounterContext,
  EncounterEligibility,
  EncounterLaunchPayload,
  EncounterResolution,
  EncounterSession,
  EncounterType,
  NodeEncounterView,
...
export class PerbugWorldService {
  private readonly registry: PerbugEncounterRegistry;
  private readonly resolver = new NodeEncounterResolver();

  constructor(private readonly store: PerbugWorldStore, registry?: PerbugEncounterRegistry) {
  ...
  previewEncounter(userId: string, nodeId: string, nodePool: PerbugNode[]): NodeEncounterView {
    ...
    const payload: EncounterLaunchPayload = {
      nodeId: node.id,
  ...
  launchEncounter(userId: string, nodeId: string, nodePool: PerbugNode[]): PlayerWorldState {
    const state = this.getPlayerState(userId);
    const node = this.requireNode(nodePool, nodeId);
    if (state.currentNodeId !== nodeId) throw new ValidationError(["player must move to node before launch"]);

    const context = this.buildContext(state, node);
    const selection = this.resolver.resolveType(context);
    const eligibility = this.computeEligibility(state, node, selection.type);
    if (!eligibility.eligible) throw new ValidationError([`encounter not eligible: ${eligibility.reasons.join(", ")}`]);
  ...
  resolveEncounter(userId: string, succeeded?: boolean): PlayerWorldState {
    ...
    const inferredResolution: EncounterResolution = succeeded === false
  ...
  abandonEncounter(userId: string): PlayerWorldState {
    const state = this.getPlayerState(userId);
    const session = this.requireActiveEncounter(state);
    if (!["active", "paused"].includes(session.state)) throw new ValidationError(["only active encounter can be abandoned"]);
    const updated = {
      ...session,
      state: "abandoned" as const,
      updatedAt: nowIso(),
      failureReason: "abandoned" as const,
      debug: {
        ...session.debug,
        transitions: [...session.debug.transitions, { from: session.state, to: "abandoned" as const, at: nowIso(), reason: "player abandoned" }]
      }
    };
  ...
  retryEncounter(userId: string): PlayerWorldState {
    const state = this.getPlayerState(userId);
    const session = this.requireActiveEncounter(state);
    if (!["failed", "abandoned"].includes(session.state)) throw new ValidationError(["retry is only allowed after fail/abandon"]);
    const reset = {
      ...session,
      ...
      debug: {
        ...session.debug,
        transitions: [...session.debug.transitions, { from: session.state, to: "active" as const, at: nowIso(), reason: "retry" }]
      }
    };
  ...
  private finalizeEncounterWithOutcome(
    ...
    if (session.rewardGranted) throw new ValidationError(["encounter rewards already granted"]);
    ...
    const updatedSession: EncounterSession = {
      ...
      rewardGranted: true,
      failureReason: succeeded ? undefined : (failureReason as any) ?? "rules_failed",
      updatedAt: nowIso(),
  ...
  private computeEligibility(state: PlayerWorldState, node: PerbugNode, type: EncounterType): EncounterEligibility {
    const reasons: string[] = [];
    if (node.state === "locked") reasons.push("node locked");
    if (state.currentNodeId !== node.id) reasons.push("player not at node");
    if (state.energy <= 0) reasons.push("insufficient energy");
    if (type === "boss" && state.squad.units.filter((u) => u.equipped).length < 2) reasons.push("squad too small for boss");
    return { eligible: reasons.length === 0, reasons };
  }

  private buildContext(state: PlayerWorldState, node: PerbugNode): EncounterContext {
    return {
      userId: state.userId,
      node,
  ...
  private bumpNodeProgress(current: NodeProgressState | undefined, nodeId: string, completionState: NodeProgressState["completionState"]): NodeProgressState {
    return {
      nodeId,
  ...
  private requireNode(nodePool: PerbugNode[], nodeId: string): PerbugNode {
    const node = nodePool.find((n) => n.id === nodeId);
    if (!node) throw new ValidationError(["node not found"]);
    return node;
  }

  private requireNodeFromState(nodeId: string, currentNodeId: string, state: PlayerWorldState, resolution: EncounterResolution): PerbugNode {
  ...
  private requireNodeByVisited(state: PlayerWorldState, nodeId: string): PerbugNode {
    return {
      id: nodeId,
  ...
  private recordEvent(state: PlayerWorldState, event: Omit<EncounterAnalyticsEvent, "id" | "timestamp" | "userId">): void {
    state.analyticsLog = [
      ...state.analyticsLog,
      {
        id: `ana_${randomUUID()}`,
  ...
  private requireActiveEncounter(state: PlayerWorldState): EncounterSession {
    if (!state.activeEncounter) throw new ValidationError(["no active encounter"]);
