import type {
  ...
  MarketplaceFilters,
  MarketplaceListing,
  MarketplaceProvenanceTag,
  MarketplaceRarity,
  MarketplaceSearchResult,
  MarketplaceSort,
} from "./types.js";

function parsePrice(listing: MarketplaceListing): number {
...
export class PerbugMarketplaceService {
  private listingViews = 0;
  private detailOpens = 0;
  private readonly categoryPopularity: Partial<Record<MarketplaceAssetType, number>> = {};
  private readonly filterUsage: Record<string, number> = {};

  constructor(private readonly listings: MarketplaceListing[]) {}

  listListings(input: { filters?: MarketplaceFilters; sort?: MarketplaceSort } = {}): MarketplaceSearchResult {
    ...
    const filtered = this.listings.filter((listing) => {
      ...
      if (!filters.status && listing.status !== "active") return false;
      if (filters.category && listing.assetType !== filters.category) return false;
      if (filters.rarity && listing.rarity !== filters.rarity) return false;
      if (filters.provenanceTag && !listing.provenance.tags.includes(filters.provenanceTag)) return false;
      if (typeof filters.minPrice === "number" && parsePrice(listing) < filters.minPrice) return false;
      if (typeof filters.maxPrice === "number" && parsePrice(listing) > filters.maxPrice) return false;
      if (filters.search && !matchesSearch(listing, filters.search)) return false;
      return true;
    });
    ...
    const sorted = [...filtered].sort((a, b) => {
      switch (sort) {
        ...
        case "rarity_desc": {
          const order: Record<MarketplaceRarity, number> = { common: 0, uncommon: 1, rare: 2, epic: 3, legendary: 4, mythic: 5 };
    ...
    const facets = {
      byType: { character: 0, item: 0, relic: 0, cosmetic: 0, map_asset: 0, special_event_asset: 0, nft_backed_asset: 0 } as Record<MarketplaceAssetType, number>,
      byRarity: { common: 0, uncommon: 0, rare: 0, epic: 0, legendary: 0, mythic: 0 } as Record<MarketplaceRarity, number>,
      byProvenance: {} as Partial<Record<MarketplaceProvenanceTag, number>>,
    };
    ...
    for (const key of filterFlags) this.filterUsage[key] = (this.filterUsage[key] ?? 0) + 1;
  ...
  analyticsSnapshot(): MarketplaceAnalyticsSnapshot {
    const rarityDistribution: Partial<Record<MarketplaceRarity, number>> = {};
    const provenanceDistribution: Partial<Record<MarketplaceProvenanceTag, number>> = {};
    const avgPriceRaw: Record<string, { sum: number; count: number }> = {};
    let mapLinkedListingCount = 0;

    for (const listing of this.listings) {
      rarityDistribution[listing.rarity] = (rarityDistribution[listing.rarity] ?? 0) + 1;
      for (const tag of listing.provenance.tags) provenanceDistribution[tag] = (provenanceDistribution[tag] ?? 0) + 1;
      if (listing.metadata.mapLink) mapLinkedListingCount += 1;
    ...
    const averagePriceByCurrency = Object.fromEntries(
      Object.entries(avgPriceRaw).map(([key, value]) => [key, Number((value.sum / value.count).toFixed(2))]),
    );
