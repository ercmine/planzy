...
describe("geocoding http endpoints", () => {
  ...
  let nominatimServer: ReturnType<typeof createNodeServer>;
  let baseUrl: string;
  ...
  beforeAll(async () => {
    ...
    await new Promise<void>((resolve) => nominatimServer.listen(0, "127.0.0.1", () => resolve()));
    ...
    if (!srvAddr || typeof srvAddr === "string") throw new Error("no server addr");
  ...
  afterAll(async () => {
    ...
    await new Promise<void>((resolve, reject) => nominatimServer.close((err) => err ? reject(err) : resolve()));
  });
