...
import type { CollaborativeGoalDefinition, SocialActionEvent, SocialPrivacySettings } from "./types.js";

function requireUserId(req: IncomingMessage) {
...
function requireAdmin(req: IncomingMessage) {
  ...
  if (!userId) throw new ValidationError(["x-admin-user-id header required"]);
}

export function createSocialGamificationHttpHandlers(service: SocialGamificationService) {
  return {
    feed: async (req: IncomingMessage, res: ServerResponse, url: URL) => {
      const userId = requireUserId(req);
      sendJson(res, 200, service.getFeed(userId, url.searchParams.get("cityId") ?? undefined));
    },
    privacy: async (req: IncomingMessage, res: ServerResponse) => {
      const userId = requireUserId(req);
      sendJson(res, 200, service.getPrivacy(userId));
    },
    updatePrivacy: async (req: IncomingMessage, res: ServerResponse) => {
      const userId = requireUserId(req);
      const body = await parseJsonBody(req);
      const payload = (typeof body === "object" && body !== null ? body : {}) as Partial<SocialPrivacySettings>;
    ...
    recordAction: async (req: IncomingMessage, res: ServerResponse) => {
      const userId = requireUserId(req);
      const body = await parseJsonBody(req);
      const payload = (typeof body === "object" && body !== null ? body : {}) as Record<string, unknown>;
      const eventId = String(payload.eventId ?? "");
      const type = String(payload.type ?? "");
      if (!eventId || !type) throw new ValidationError(["eventId and type are required"]);
      const event: SocialActionEvent = {
        eventId,
        actorUserId: userId,
        type: type as SocialActionEvent["type"],
        ...
        contentState: payload.contentState == null ? undefined : String(payload.contentState) as SocialActionEvent["contentState"],
    ...
    upsertGoal: async (req: IncomingMessage, res: ServerResponse) => {
      requireAdmin(req);
      const body = await parseJsonBody(req);
      if (typeof body !== "object" || body === null) throw new ValidationError(["goal payload required"]);
      sendJson(res, 200, service.upsertGoal(body as CollaborativeGoalDefinition));
    }
  };
}
