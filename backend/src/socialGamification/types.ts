...
export interface FriendChallengeInstance {
  ...
  startsAt: string;
  endsAt: string;
  participantProgress: Record<string, { points: number; distinctPlaceIds: string[]; completedAt?: string }>;
  winnerUserId?: string;
  trustFlags: string[];
  createdAt: string;
  updatedAt: string;
...
export interface CollaborativeGoalDefinition {
  id: string;
  title: string;
  scope: { cityId: string; neighborhoodId?: string; categoryId?: string };
  visibility: SocialVisibility;
  startsAt: string;
  endsAt: string;
  targetPoints: number;
...
export interface SocialFeedResponse {
  ...
  collaborativeGoals: Array<CollaborativeGoalDefinition & { progress: CollaborativeGoalProgress }>;
