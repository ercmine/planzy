...
export function createPerbugRewardsHttpHandlers(service: PerbugRewardsService) {
  return {
    ...
    submitReview: async (req: IncomingMessage, res: ServerResponse) => {
      const body = await parseJsonBody(req) as { placeId?: string; videoUrl?: string; contentHash?: string; qualityRating?: "low"|"standard"|"high"|"featured" };
      if (!body.placeId || !body.videoUrl || !body.contentHash) throw new ValidationError(["placeId, videoUrl, contentHash required"]);
      sendJson(res, 201, { review: service.submitReview({ userId: requireUserId(req), placeId: body.placeId, videoUrl: body.videoUrl, contentHash: body.contentHash, qualityRating: body.qualityRating }) });
    },
    ...
    claim: async (req: IncomingMessage, res: ServerResponse, reviewId: string) => {
      ...
      sendJson(res, 200, await service.claimReward({ userId: requireUserId(req), reviewId, walletPublicKey: body.walletPublicKey, idempotencyKey: body.idempotencyKey }));
    },
