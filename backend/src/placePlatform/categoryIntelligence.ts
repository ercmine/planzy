...
export const OSM_CATEGORY_RULES: SourceCategoryMappingRule[] = [
...
const CUISINE_REFINEMENTS: Array<{ token: string; categoryId: string; confidence: number; priority: number }> = [
...
export class OsmCategoryNormalizationEngine {
  constructor(private readonly rules: SourceCategoryMappingRule[]) {}

  normalize(tags: Record<string, string>): CategoryNormalizationResult {
    const directMatches: CategoryNormalizationMatch[] = this.rules
      .filter((rule) => {
    ...
    const deduped: CategoryNormalizationMatch[] = [];
