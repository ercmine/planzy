...
describe("collaboration routes", () => {
  ...
  it("supports invite, accept, campaign linkage and featured listing", async () => {
    ...
    const creator = await creatorRes.json() as { profile: { id: string } };
    ...
    const claim = await claimDraft.json() as { id: string };
    ...
    const invite = await inviteRes.json() as { invite: { id: string; campaignId?: string } };
    ...
    const listed = await businessInvites.json() as { invites: Array<{ campaignId?: string }> };
