...
describe("merchant admin routes", () => {
  it("enforces admin key and CRUDs promoted and specials", async () => {
    ...
    const createdPromotedBody = (await createPromoted.json()) as { promoId: string };
    ...
    const promotedListBody = (await listPromoted.json()) as { items: Array<{ promoId: string }> };
    ...
    const patchedPromotedBody = (await patchPromoted.json()) as { status: string };
    ...
    const createdSpecialBody = (await createSpecial.json()) as { specialId: string };
    ...
    const specialsListBody = (await listSpecials.json()) as { items: Array<{ specialId: string }> };
    ...
    const patchedSpecialBody = (await patchSpecial.json()) as { status: string };
