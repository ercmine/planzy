...
describe('competition routes', () => {
  it('serves home, missions, leaderboards, quality, and rewards flows', async () => {
    ...
    const homeJson = await home.json() as { missions: Array<{ id: string }>; rewards: Array<{ id: string }>; leaderboards: Array<{ id: string }> };
    ...
    const qualityJson = await quality.json() as { quality: { earlyLikeCount: number } };
    ...
    const missionClaimJson = await missionClaim.json() as { reward: { id: string } };
