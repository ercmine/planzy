...
describe("dryad reward routes", () => {
  it("supports reward previews, review approval ladder, and idempotent claims", async () => {
    ...
    expect(((await preview.json()) as { nextBaseRewardAmount: number }).nextBaseRewardAmount).toBe(200);
    ...
    const review6Ids: string[] = [];
    const firstId = ((await review1.json()) as { review: { id: string } }).review.id;
    const secondId = ((await review2.json()) as { review: { id: string } }).review.id;
    ...
    for (let i = 3; i <= 6; i += 1) {
      ...
      const id = ((await review.json()) as { review: { id: string } }).review.id;
      review6Ids.push(id);
    ...
    const nonceBody = await nonceResponse.json() as { message: string };
    ...
    const claimA = await claimFirst.json() as { claim: { transactionSignature: string } };
    const claimB = await claimSecond.json() as { claim: { transactionSignature: string } };
    ...
    const dashboardBody = await dashboard.json() as { history: Array<{ review: { rewardStatus: string } }> };
