...
afterEach(async () => {
  await Promise.all(serversToClose.splice(0).map((server) => new Promise<void>((resolve, reject) => {
...
describe("rollout routes", () => {
  it("returns rollout summary and blocks gated outing planner endpoints", async () => {
    ...
    if (!addr || typeof addr === "string") throw new Error("invalid server addr");
    const base = `http://127.0.0.1:${addr.port}`;

    const summary = await fetch(`${base}/v1/rollouts/summary`, { headers: { "x-user-id": "user-1" } });
    expect(summary.status).toBe(200);
    const summaryBody = await summary.json() as { features: Record<string, { enabled: boolean }> };
    ...
    const blockedBody = await blocked.json() as { error: string; featureKey: string };
