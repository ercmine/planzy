...
describe("business review responses", () => {
  ...
  it("enforces verified ownership and exposes only public business response payload", async () => {
    ...
    const reviewBody = await reviewRes.json() as { review: { id: string } };
    ...
    const responsePayload = await createResponseRes.json() as { response: { id: string; visibilityStatus: string } };
    ...
    const placeReviews = await placeReviewsRes.json() as { reviews: Array<{ businessResponse?: { content: string; attributionLabel: string; moderationStatus?: string } }> };
