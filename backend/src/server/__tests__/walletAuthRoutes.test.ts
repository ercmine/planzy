...
describe("wallet auth routes", () => {
  it("creates challenge, verifies signature, restores session, and logs out", async () => {
    ...
    const verified = await verifyRes.json() as { sessionToken: string; authState: string };
    ...
    expect(((await restore.json()) as { authState: string }).authState).toBe("authenticated");
    ...
    expect(((await after.json()) as { authState: string }).authState).toBe("signed_out");
  });
});
