...
import { ProfileType, UserRole, type UserIdentity } from "../accounts/types.js";
...
import { MemoryRolloutStore, type RolloutStore } from "./store.js";
import type { AccountTypeRollout, RolloutContext, RolloutDecision, RolloutDefinition, RolloutEnvironment } from "./types.js";

const nowIso = () => new Date().toISOString();

function normalizeEnv(value: string | undefined): RolloutEnvironment {
  if (!value) return "development";
  if (value === "prod" || value === "production") return "production";
  if (value === "stage" || value === "staging") return "staging";
  if (value === "preview") return "preview";
  if (value === "local") return "local";
  return "development";
}

function defaultDefinitions(): RolloutDefinition[] {
  const at = nowIso();
...
export class RolloutAccessError extends Error {
  constructor(public readonly decision: RolloutDecision) {
...
export class RolloutService {
  constructor(
    private readonly store: RolloutStore = new MemoryRolloutStore(defaultDefinitions()),
    private readonly accounts?: AccountsService,
    private readonly subscriptions?: SubscriptionService,
    private readonly environment: RolloutEnvironment = normalizeEnv(process.env.NODE_ENV)
  ) {}
  ...
  evaluate(featureKey: string, context: Omit<RolloutContext, "featureKey">): RolloutDecision {
    if (!ALL_ROLLOUT_FEATURE_KEYS.includes(featureKey as any)) {
    ...
    if (!rule) return { featureKey, enabled: false, reason: "global_off", environment: context.environment };
    ...
    if (rule.status === "off") return { featureKey, enabled: false, reason: "global_off", environment: context.environment };
    ...
    if (rule.denyUserIds?.includes(context.userId ?? "")) return { featureKey, enabled: false, reason: "denied_user", environment: context.environment };
    if (rule.denyCohorts?.some((c) => context.cohorts.includes(c))) return { featureKey, enabled: false, reason: "denied_cohort", environment: context.environment };
    if (rule.denyMarkets?.includes(context.market ?? "")) return { featureKey, enabled: false, reason: "denied_market", environment: context.environment };
  ...
  assertFeatureRolledOut(featureKey: string, context: Omit<RolloutContext, "featureKey">): void {
    ...
    if (!decision.enabled) throw new RolloutAccessError(decision);
  }
  ...
  listDefinitions(): RolloutDefinition[] { return this.store.list(); }

  updateDefinition(definition: Omit<RolloutDefinition, "updatedAt"> & { updatedAt?: string }): RolloutDefinition {
    const next: RolloutDefinition = { ...definition, updatedAt: definition.updatedAt ?? nowIso() };
  ...
  private percentageBucket(featureKey: string, id: string, salt: string): number {
    const digest = createHash("sha256").update(`${featureKey}:${salt}:${id}`).digest("hex").slice(0, 8);
    return Number.parseInt(digest, 16) % 100;
  }

  private deriveAccountType(user?: UserIdentity, activeProfileType?: ProfileType): AccountTypeRollout {
    if (activeProfileType === ProfileType.BUSINESS) return "business";
    if (activeProfileType === ProfileType.CREATOR) return "creator";
    if (user?.activeProfileType === ProfileType.BUSINESS) return "business";
    if (user?.activeProfileType === ProfileType.CREATOR) return "creator";
    return "user";
  }
}
...
export function rolloutErrorPayload(error: RolloutAccessError) {
  return {
