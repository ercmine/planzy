...
export function createPerbugEconomyHttpHandlers(service: PerbugEconomyService) {
  return {
    upsertPayoutAddress: async (req: IncomingMessage, res: ServerResponse) => {
      const body = await parseJsonBody(req) as { payoutAddress?: string };
      sendJson(res, 200, { payoutProfile: service.upsertPayoutAddress(requireHeader(req, "x-user-id"), String(body.payoutAddress ?? "")) });
    },
    withdrawals: async (req: IncomingMessage, res: ServerResponse) => {
    ...
    withdraw: async (req: IncomingMessage, res: ServerResponse) => {
      const body = await parseJsonBody(req) as { toAddress?: string; amountPerbug?: number; idempotencyKey?: string };
