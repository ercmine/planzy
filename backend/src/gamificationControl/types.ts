...
export interface MechanicsConfig {
  xp: {
    maxDailyXp: number;
    trustMultipliers: Array<{ minTrustScore: number; multiplier: number }>;
    rules: XpRule[];
  };
  badges: BadgeRule[];
  quests: QuestRule[];
  ...
  leaderboard: {
    ...
    weights: Array<{ actionType: ActionType; weight: number; qualityWeight: number }>;
  };
  antiAbuse: {
...
export interface RewardDecision {
  ...
  streakMilestonesReached: Array<{ streakId: string; days: number }>;
...
export interface UserGamificationState {
  ...
  completedQuestIds: string[];
  streaks: Record<string, { lastEventDate?: string; currentDays: number }>;
...
export interface ProgressionSummaryDto {
  ...
  questProgress: Record<string, { current: number; completed: boolean }>;
  leaderboardScore: number;
