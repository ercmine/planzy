...
const FIT_THRESHOLDS: Record<CategoryStrictnessMode, Record<CategoryFitLabel, number>> = {
  strict: {
    exact_match: 0.9,
    strong_match: 0.76,
    decent_match: 0.62,
...
const PROVIDER_TYPE_MAPPINGS: Record<ProviderId, Record<string, Array<{ categoryId: string; weight: number }>>> = {
  google: {
...
function toSearchText(evidence: ProviderPlaceEvidence): string {
  return [
    evidence.name,
...
function fitFromConfidence(confidence: number, strictness: CategoryStrictnessMode): CategoryFitLabel {
...
function addContribution(
  target: Map<string, CategoryEvidenceBreakdown>,
  categoryId: string,
  contribution: CategoryEvidenceContribution,
  bucket: "contributions" | "bonuses" | "penalties"
): void {
  const existing =
...
function scoreProviderMappings(evidenceList: ProviderPlaceEvidence[], scores: Map<string, number>, breakdown: Map<string, CategoryEvidenceBreakdown>): void {
    ...
    for (const rawType of allTypes) {
      ...
      for (const candidate of mapped) {
        ...
        addContribution(
          ...
          { source: `${evidence.provider}:taxonomy`, value: rawType, delta: candidate.weight },
          "contributions"
        );
      }
    }
...
function scoreKeywordSignals(
  evidenceList: ProviderPlaceEvidence[],
  scores: Map<string, number>,
  breakdown: Map<string, CategoryEvidenceBreakdown>
): void {
...
function applySpecificityAndAgreement(
  evidenceList: ProviderPlaceEvidence[],
  scores: Map<string, number>,
  breakdown: Map<string, CategoryEvidenceBreakdown>
): void {
  ...
  for (const definition of CATEGORY_DEFINITIONS) {
    ...
    if (childScore > 0.65 && parentScore >= childScore * 0.9) {
      ...
      addContribution(
        breakdown,
        definition.parentId,
        { source: "penalty:parent_competition", value: definition.id, delta: -dampen },
        "penalties"
      );
    }
  }
...
function applyManualOverrides(
  override: ManualCategoryOverride | undefined,
  scores: Map<string, number>,
  breakdown: Map<string, CategoryEvidenceBreakdown>,
  flags: string[]
): void {
...
export function classifyPlaceCategories(
  evidenceList: ProviderPlaceEvidence[],
  override?: ManualCategoryOverride
): PlaceCategoryProfile {
  const scores = new Map<string, number>();
  const breakdown = new Map<string, CategoryEvidenceBreakdown>();
  const flags: string[] = [];
  ...
  const candidates: CategoryCandidate[] = CATEGORY_DEFINITIONS.map((definition) => {
  ...
  const fitByCategory: Record<string, CategoryFitLabel> = {};
  const evidenceByCategory: Record<string, CategoryEvidenceBreakdown> = {};
...
function googlePlaceToEvidence(place: GooglePlace): ProviderPlaceEvidence {
  return {
    provider: "google",
    placeId: place.id,
