...
const DEFAULT_CONFIG: PlaceDataQualityConfig = {
...
function issueKey(issueType: PlaceDataQualityIssueType, placeId: string, suffix?: string): string {
...
function distKm(a: CanonicalPlace, b: CanonicalPlace): number {
...
export class PlaceDataQualityService {
  private readonly issuesByKey = new Map<string, PlaceDataQualityIssue>();
  constructor(private readonly config: PlaceDataQualityConfig = DEFAULT_CONFIG, private readonly now: () => number = () => Date.now()) {}

  evaluate(places: CanonicalPlace[], sourceRecords: PlaceSourceRecord[]): PlaceDataQualityIssue[] {
    const next = new Map<string, PlaceDataQualityIssue>();
    const recordsByPlace = new Map<string, PlaceSourceRecord[]>();
    for (const record of sourceRecords) {
      if (!record.canonicalPlaceId) continue;
    ...
    for (const [key, value] of next) this.issuesByKey.set(key, value);
    return [...this.issuesByKey.values()];
  }

  listIssues(filters: PlaceQualityFilters = {}, page = 0, pageSize = 50) {
    const filtered = [...this.issuesByKey.values()].filter((issue) => {
      ...
      if (filters.provider && issue.provider !== filters.provider) return false;
      if (filters.city && issue.city !== filters.city) return false;
      if (filters.category && issue.category !== filters.category) return false;
      if (filters.placeId && issue.placeId !== filters.placeId) return false;
      return true;
  ...
  summarize() {
    ...
    const countBy = <T extends string>(selector: (issue: PlaceDataQualityIssue) => T | undefined) => {
  ...
  getProviderSummary() {
    const grouped = new Map<string, { provider: string; total: number; open: number; byType: Record<string, number> }>();
    for (const issue of this.issuesByKey.values()) {
      ...
      row.total += 1;
      if (issue.status === "open" || issue.status === "acknowledged") row.open += 1;
  ...
  updateIssueStatus(issueId: string, status: PlaceDataQualityIssueStatus, actorUserId: string, note?: string) {
    ...
    const updated: PlaceDataQualityIssue = {
      ...issue,
      status,
  ...
  private mergeIssue(next: PlaceDataQualityIssue): PlaceDataQualityIssue {
  ...
  private detectForPlace(place: CanonicalPlace, records: PlaceSourceRecord[]): PlaceDataQualityIssue[] {
    const out: PlaceDataQualityIssue[] = [];
    const now = this.now();
  ...
  private detectDuplicates(places: CanonicalPlace[]): PlaceDataQualityIssue[] {
    const out: PlaceDataQualityIssue[] = [];
    for (let i = 0; i < places.length; i += 1) {
      for (let j = i + 1; j < places.length; j += 1) {
        const a = places[i]!;
        const b = places[j]!;
        if (a.locality && b.locality && a.locality !== b.locality) continue;
        const distance = distKm(a, b);
        if (distance > this.config.duplicateRadiusKm) continue;
        let score = 0;
  ...
  private buildIssue(place: CanonicalPlace, issueType: PlaceDataQualityIssueType, severity: PlaceDataQualityIssueSeverity, input: {
    provider?: string;
    confidence?: number;
    evidence: Record<string, unknown>;
  }, suffix?: string): PlaceDataQualityIssue {
    const nowIso = new Date(this.now()).toISOString();
    return {
      id: `dq_${randomUUID()}`,
...
export function createPlaceDataQualityConfigFromEnv(env: NodeJS.ProcessEnv = process.env): PlaceDataQualityConfig {
  return {
    ...DEFAULT_CONFIG,
