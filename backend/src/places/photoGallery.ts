...
function buildPhotoFingerprint(provider: string, photo: NormalizedProviderPhoto): string {
...
function isWeakAsset(photo: CanonicalPhoto): boolean {
  const type = (photo.photoType ?? "other").toLowerCase();
  return type === "logo" || type === "map";
}

function pixelScore(photo: CanonicalPhoto): number {
  return (photo.width ?? 0) * (photo.height ?? 0);
}

function rankPhoto(photo: CanonicalPhoto): number {
...
export function normalizeProviderPhoto(params: {
  ...
  const canonical: CanonicalPhoto = {
...
export function dedupeAndRankPhotos(photos: CanonicalPhoto[]): CanonicalPhoto[] {
  const deduped = new Map<string, CanonicalPhoto>();
...
export function selectBestPhotoUrl(photo: CanonicalPhoto, preferred: "thumbnail" | "medium" | "full" = "medium"): string | undefined {
