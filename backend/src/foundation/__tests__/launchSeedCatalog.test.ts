...
describe("launch readiness seed catalog", () => {
  it("includes representative launch account, moderation, and ad fixture states", () => {
    ...
    const json = JSON.parse(readFileSync(file, "utf8")) as {
      users: Array<{ id: string; state: string; plan: string; roles: string[] }>;
      places: Array<{ id: string; photoCount: number; reviewCount: number }>;
      moderationCases: Array<{ targetType: string; state: string }>;
      ads: { placements: string[]; fallbackCreative: string };
    };
