...
import type {
  ...
  CompetitionMissionWithProgress,
  CompetitionReward,
...
const DEFAULT_CONFIG: CompetitionScoringConfig = {
...
export class CompetitionService {
  ...
  getLeaderboard(id: string) { this.rebuildLeaderboards(); return { leaderboard: this.requireLeaderboard(id), entries: this.store.listLeaderboardEntries(id) }; }
  getMissionProgress(missionId: string, userId: string) { return this.ensureProgress(missionId, userId); }
  listRewards(userId: string) { return this.store.listRewards(userId); }
  getVideoQuality(videoId: string) { return this.finalizeQualityForVideo(videoId); }
  ...
  claimMission(missionId: string, userId: string): CompetitionReward {
    ...
    if (!progress.completed) throw new ValidationError(["mission not complete"]);
    if (progress.claimed) {
      const existing = this.store.listRewards(userId).find((reward) => reward.sourceType === "mission" && reward.sourceId === missionId);
      if (!existing) throw new ValidationError(["mission already claimed"]);
      return existing;
    }
  ...
  claimReward(rewardId: string, userId: string): CompetitionReward {
    ...
    if (!reward || reward.userId !== userId) throw new ValidationError(["reward not found"]);
    if (reward.status === "claimed") return reward;
    if (reward.status !== "claimable") throw new ValidationError(["reward is not claimable"]);
  ...
  createMission(input: Omit<CompetitionMission, "createdAt" | "updatedAt">, actorUserId = "admin") {
  ...
  createLeaderboard(input: Omit<CompetitionLeaderboard, "createdAt" | "updatedAt">, actorUserId = "admin") {
    const timestamp = nowIso(this.nowProvider());
  ...
  blockReward(rewardId: string, actorUserId = "admin") {
    const reward = this.store.getReward(rewardId);
  ...
  recomputeVideoQuality(videoId: string, actorUserId = "admin") {
  ...
  recordVideoPublished(input: { videoId: string; reviewId?: string; userId: string; publishedAt?: string; city?: string; category?: string; canonicalPlaceId?: string }) {
    const publishedAt = input.publishedAt ?? nowIso(this.nowProvider());
    const timestamp = nowIso(this.nowProvider());
    const snapshot: CompetitionVideoQualitySnapshot = {
      id: `cq_${randomUUID()}`,
  ...
  recordLike(event: CompetitionLikeEvent) {
  ...
  recordApprovedReview(event: CompetitionReviewEvent) {
  ...
  recordTip(event: CompetitionTipEvent) {
  ...
  updateStreak(userId: string, streakDays: number) {
    const profile = this.store.getUserProfile(userId) ?? { userId, streakDays: 0 };
  ...
  private attachMissionProgress(mission: CompetitionMission, userId: string): CompetitionMissionWithProgress {
    return { ...mission, progress: this.ensureProgress(mission.id, userId) };
  }

  private ensureProgress(missionId: string, userId: string): CompetitionMissionProgress {
    const existing = this.store.getMissionProgress(missionId, userId);
    if (existing) return existing;
    const timestamp = nowIso(this.nowProvider());
    const progress: CompetitionMissionProgress = { id: `cmp_${randomUUID()}`, missionId, userId, progressValue: 0, completed: false, claimed: false, createdAt: timestamp, updatedAt: timestamp };
    this.store.saveMissionProgress(progress);
    return progress;
  }

  private finalizeQualityForVideo(videoId: string, force = false): CompetitionVideoQualitySnapshot | null {
    ...
    if (!force && snapshot.finalized && Date.parse(snapshot.qualityWindowEndsAt) <= this.nowProvider().getTime()) return snapshot;

    const deduped = new Map<string, CompetitionLikeEvent>();
    for (const like of this.store.listLikeEvents(videoId)) {
      if (!like.valid || like.bannedUser || like.blockedUser || like.fraudFlagged) continue;
      if (Date.parse(like.createdAt) > Date.parse(snapshot.qualityWindowEndsAt)) continue;
      const existing = deduped.get(like.userId);
      if (!existing || Date.parse(existing.createdAt) > Date.parse(like.createdAt)) deduped.set(like.userId, like);
    }
  ...
  private rebuildLeaderboards() {
    ...
    for (const board of boards) {
      const entries = new Map<string, CompetitionLeaderboardEntry>();
      ...
      const users = new Set<string>([...activeReviews.map((event) => event.userId), ...activeQuality.map((snapshot) => snapshot.userId), ...activeTips.map((tip) => tip.userId)]);
      ...
      if (season && new Date(board.endsAt).getTime() < this.nowProvider().getTime()) {
        for (const entry of withRewards.filter((item) => (item.rewardAtomic ?? 0n) > 0n)) {
          ...
          if (!existing) this.issueReward({ userId: entry.userId, sourceType: "leaderboard", sourceId: `${board.id}:${entry.userId}`, rewardAtomic: entry.rewardAtomic ?? 0n });
        }
      }
    }
  }

  private computeUserScore(userId: string, board?: CompetitionLeaderboard): number {
  ...
  private updateMissionProgressForAllUsers() {
    ...
    for (const userId of users) {
      for (const mission of this.listMissions()) {
        ...
        if (progress.completed && !progress.completedAt) progress.completedAt = nowIso(this.nowProvider());
  ...
  private measureMission(userId: string, mission: CompetitionMission): number {
    ...
    switch (mission.type) {
      ...
      case "tips_received": return Number(tips.reduce((sum, tip) => sum + tip.amountAtomic, 0n) / 1_000_000_000n);
  ...
  private issueReward(input: { userId: string; sourceType: CompetitionReward["sourceType"]; sourceId: string; rewardAtomic: bigint }): CompetitionReward {
    ...
    if (existing) return existing;
    const timestamp = nowIso(this.nowProvider());
    const reward: CompetitionReward = { id: `cr_${randomUUID()}`, userId: input.userId, sourceType: input.sourceType, sourceId: input.sourceId, rewardAtomic: input.rewardAtomic, status: "claimable", createdAt: timestamp, updatedAt: timestamp };
    this.store.saveReward(reward);
    return reward;
  }

  private matchesBoardScope(board: CompetitionLeaderboard, city?: string, category?: string) {
    if (board.scopeType === "global" || !board.scopeValue) return true;
    if (board.scopeType === "city") return board.scopeValue === city;
    if (board.scopeType === "category") return board.scopeValue === category;
    return true;
  }

  private withinWindow(dateIso: string, startsAt: string, endsAt: string) {
    const value = Date.parse(dateIso);
    return value >= Date.parse(startsAt) && value <= Date.parse(endsAt);
  }

  private requireMission(id: string) { const mission = this.store.getMission(id); if (!mission) throw new ValidationError(["mission not found"]); return mission; }
  private requireLeaderboard(id: string) { const leaderboard = this.store.getLeaderboard(id); if (!leaderboard) throw new ValidationError(["leaderboard not found"]); return leaderboard; }
  private audit(actorUserId: string, action: string, targetType: string, targetId: string, payload: Record<string, unknown>): CompetitionAuditLog {
    const log: CompetitionAuditLog = { id: `cal_${randomUUID()}`, actorUserId, action, targetType, targetId, payload, createdAt: nowIso(this.nowProvider()) };
    this.store.addAuditLog(log);
    return log;
  }

  private seedDefaults() {
    if (this.store.listSeasons().length) return;
    const now = this.nowProvider();
    const weekStart = new Date(now); weekStart.setUTCDate(now.getUTCDate() - ((now.getUTCDay() + 6) % 7)); weekStart.setUTCHours(0,0,0,0);
    ...
    const boards: CompetitionLeaderboard[] = [
      { id: "lb_weekly_global", type: "weekly_global", name: "Global weekly leaderboard", scopeType: "global", startsAt: weekStart.toISOString(), endsAt: weekEnd.toISOString(), rewardPoolAtomic: 600_000_000n, scoringRuleVersion: "v1", active: true, createdAt: timestamp, updatedAt: timestamp },
      { id: "lb_weekly_city", type: "weekly_city", name: "Bloomington weekly leaderboard", scopeType: "city", scopeValue: "Bloomington", startsAt: weekStart.toISOString(), endsAt: weekEnd.toISOString(), rewardPoolAtomic: 300_000_000n, scoringRuleVersion: "v1", active: true, createdAt: timestamp, updatedAt: timestamp },
      { id: "lb_weekly_quality", type: "weekly_quality", name: "Most liked in first 48h", scopeType: "global", startsAt: weekStart.toISOString(), endsAt: weekEnd.toISOString(), rewardPoolAtomic: 250_000_000n, scoringRuleVersion: "v1", active: true, createdAt: timestamp, updatedAt: timestamp },
      { id: "lb_weekly_discovery", type: "weekly_discovery", name: "Discovery leaderboard", scopeType: "global", startsAt: weekStart.toISOString(), endsAt: weekEnd.toISOString(), rewardPoolAtomic: 250_000_000n, scoringRuleVersion: "v1", active: true, createdAt: timestamp, updatedAt: timestamp },
      { id: "lb_weekly_tips", type: "weekly_most_tipped", name: "Most tipped creator leaderboard", scopeType: "global", startsAt: weekStart.toISOString(), endsAt: weekEnd.toISOString(), rewardPoolAtomic: 200_000_000n, scoringRuleVersion: "v1", active: true, createdAt: timestamp, updatedAt: timestamp },
      { id: "lb_seasonal_overall", type: "seasonal_overall", name: "Seasonal overall score", scopeType: "season", scopeValue: "season_current", startsAt: timestamp, endsAt: seasonEnd.toISOString(), rewardPoolAtomic: 900_000_000n, scoringRuleVersion: "v1", active: true, createdAt: timestamp, updatedAt: timestamp }
    ];
