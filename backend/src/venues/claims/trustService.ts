...
import type {
  BusinessClaimedStatus,
  BusinessCompletenessTier,
  BusinessContactMethodRecord,
  BusinessContactMethodType,
...
const CONTACT_PRIORITY: BusinessContactMethodType[] = ["phone", "email", "website", "booking_url", "contact_url", "social"];

export class BusinessTrustService {
  constructor(private readonly store: VenueClaimStore, private readonly now: () => Date = () => new Date()) {}

  public async recompute(placeId: string): Promise<BusinessTrustProfile> {
    ...
    if (!contacts.some((entry) => entry.verificationStatus === "verified")) warnings.push("no_verified_contact");
    if (claimedStatus !== "claimed") warnings.push("not_claimed");
    ...
    const profile: BusinessTrustProfile = {
  ...
  public async upsertContactMethod(placeId: string, input: {
    ...
    const record: BusinessContactMethodRecord = {
      id: existing?.id ?? randomUUID(),
      placeId,
  ...
  public async setContactVerificationStatus(input: {
    placeId: string;
    ...
    if (!contact || contact.placeId !== input.placeId) throw new ValidationError(["contact method not found"]);
    const nowIso = this.now().toISOString();
    const next: BusinessContactMethodRecord = {
      ...contact,
    ...
    const eventType: BusinessTrustAuditEvent["eventType"] = input.status === "verified"
  ...
  public async buildPublicTrustView(placeId: string): Promise<BusinessTrustPublicView> {
  ...
  private buildBadges(trust: BusinessTrustProfile, verifiedContactTypes: BusinessContactMethodType[]): BusinessTrustBadge[] {
    const badges: BusinessTrustBadge[] = [];
  ...
  private resolveClaimedStatus(ownership: Array<{ isActive: boolean; verificationStatus: "pending" | "verified" | "revoked"; revokedReasonCode?: string }>): BusinessClaimedStatus {
    if (ownership.some((entry) => entry.isActive && entry.verificationStatus === "verified")) return "claimed";
    if (ownership.some((entry) => entry.verificationStatus === "pending")) return "claimed_pending";
    if (ownership.some((entry) => entry.verificationStatus === "revoked")) {
      if (ownership.some((entry) => entry.revokedReasonCode === "ownership_dispute")) return "claim_disputed";
      return "claim_revoked";
    }
    return "unclaimed";
  }

  private resolveCompletenessTier(score: number): BusinessCompletenessTier {
  ...
  private normalizeContactValue(type: BusinessContactMethodType, value: string): string {
    if (type === "phone") {
      const normalized = normalizePhone(value);
      if (!normalized) throw new ValidationError(["phone contact must be valid"]);
      return normalized;
    }
    if (type === "email") {
      const normalized = value.trim().toLowerCase();
      if (!/^[-a-z0-9._+]+@[-a-z0-9.]+\.[a-z]{2,}$/i.test(normalized)) throw new ValidationError(["email contact must be valid"]);
      return normalized;
    }
    const normalizedUrl = normalizeUrl(value);
    if (!normalizedUrl) throw new ValidationError([`${type} contact must be a valid http/https url`]);
    return normalizedUrl;
  }

  private async appendAuditEvent(input: Omit<BusinessTrustAuditEvent, "id" | "createdAt">): Promise<void> {
