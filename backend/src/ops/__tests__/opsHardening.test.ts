...
import { applyRetention, evaluateBackupReadiness, type BackupRecord } from "../backup.js";
...
describe("ops hardening foundations", () => {
  ...
  it("detects queue backlog and worker heartbeat failures", () => {
    const check = evaluateQueueHealth(
      { queue: "media", depth: 600, oldestJobAgeMs: 80_000, workerLastHeartbeatMs: 100_000, failedInWindow: 12, retriesInWindow: 6 },
      { maxDepth: 200, maxOldestJobAgeMs: 30_000, maxWorkerHeartbeatLagMs: 20_000, maxFailureRate: 0.45 }
    );
  ...
  it("validates required ops secrets and catches insecure defaults", () => {
    const validation = validateOperationalConfig(
      { BACKUP_S3_BUCKET: "", BACKUP_S3_ACCESS_KEY: "changeme", REDIS_URL: "redis://cache" },
      [
