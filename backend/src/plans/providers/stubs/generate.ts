...
type WeightedCategory = { category: Category; weight: number };
...
const BASE_CATEGORY_WEIGHTS: Record<Category, number> = {
  food: 1.5,
  drinks: 1.1,
  coffee: 1.2,
  outdoors: 0.8,
...
function weightedPickCategory(rng: Rng, categories: WeightedCategory[]): Category {
...
function buildCategoryWeights(input: SearchPlansInputNormalized, opts: StubGenOptions): WeightedCategory[] {
  const requested = input.categories ?? [];
  const requestedSet = new Set(requested);

  return (Object.keys(BASE_CATEGORY_WEIGHTS) as Category[]).map((category) => {
...
function buildTitle(rng: Rng, category: Category): string {
...
function buildBaseEntities(seed: string, input: SearchPlansInputNormalized, count: number, categories: WeightedCategory[]): BaseEntity[] {
...
function fixedBaseDate(seed: string): Date {
...
function createPlan(entity: BaseEntity, input: SearchPlansInputNormalized, opts: StubGenOptions, rng: Rng, index: number): Plan {
  ...
  const maybePrice = rng() < 0.2 ? undefined : (randInt(rng, 1, 4) as 1 | 2 | 3 | 4);
  ...
  const deepLinks: NonNullable<Plan["deepLinks"]> = {
  ...
  const distanceMeters = Math.round(
    haversineMeters(input.location, {
...
export async function generatePlans(input: SearchPlansInputNormalized, opts: StubGenOptions): Promise<Plan[]> {
