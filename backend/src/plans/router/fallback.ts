...
import { isAbortError, type PlanProvider, type ProviderContext } from "../provider.js";
import type { SearchPlansInput } from "../types.js";
import { validateSearchPlansInput } from "../validation.js";
import type { FallbackDebug, FallbackResult, NeverEmptyOptions } from "./fallbackTypes.js";
...
function asAbortedError(error: unknown): ProviderError {
...
function sanitizeError(provider: string, error: unknown): { provider: string; code: string; message: string } {
...
export async function applyNeverEmptyFallback(params: {
  ...
  providerErrors: { provider: string; error: unknown }[];
  opts?: NeverEmptyOptions & { includeDebug?: boolean };
  now?: Date;
  ...
  const failedNonFallbacks = new Set(
    params.providerErrors
  ...
  let reason: FallbackDebug["reason"];
  if (shouldTrigger) {
  ...
  const order = preferByoFirst
    ? [
        { key: "byo" as const, provider: byoProvider },
        { key: "curated" as const, provider: curatedProvider }
      ]
    : [
        { key: "curated" as const, provider: curatedProvider },
        { key: "byo" as const, provider: byoProvider }
