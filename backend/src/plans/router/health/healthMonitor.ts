import type { ProviderCallOutcome, ProviderHealthConfig, ProviderHealthSnapshot, ProviderStatus } from "./healthTypes.js";
...
interface ProviderState {
  ...
  halfOpenProbeUsedUntilMs?: number;
  lastUpdatedMs: number;
}

function quantile95(values: number[]): number | undefined {
  if (values.length === 0) {
    return undefined;
  }
...
export class ProviderHealthMonitor {
  private readonly cfg: ProviderHealthConfig;
  private readonly now: () => number;
  private readonly stateByProvider = new Map<string, ProviderState>();

  constructor(cfg?: Partial<ProviderHealthConfig>, deps?: { now?: () => number }) {
    this.cfg = { ...DEFAULT_CONFIG, ...cfg };
    this.now = deps?.now ?? (() => Date.now());
  }

  public record(outcome: Omit<ProviderCallOutcome, "atMs"> & { atMs?: number }): void {
  ...
  public shouldSkip(provider: string): { skip: boolean; reason?: string; retryAfterMs?: number } {
    const now = this.now();
  ...
  public noteAttempt(provider: string): void {
    const now = this.now();
    const state = this.stateByProvider.get(provider);
  ...
  public snapshot(provider: string): ProviderHealthSnapshot | null {
    const state = this.stateByProvider.get(provider);
    if (!state) {
      return null;
    }
  ...
  public snapshots(): ProviderHealthSnapshot[] {
    const now = this.now();
  ...
  private evaluateForDisable(state: ProviderState, now: number): void {
    if (state.disabledUntilMs && now < state.disabledUntilMs) {
      return;
    }
  ...
  private disable(state: ProviderState, now: number): void {
    state.disabledUntilMs = now + this.cfg.disableForMs;
    state.halfOpenProbeUsedUntilMs = undefined;
  }

  private computeSnapshot(provider: string, state: ProviderState, now: number): ProviderHealthSnapshot {
