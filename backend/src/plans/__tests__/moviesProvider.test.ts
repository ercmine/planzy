...
describe("MoviesProvider", () => {
  ...
  it("uses Google when key present; falls back to Yelp when Google key missing", async () => {
    ...
    const yelpUrls: string[] = [];
    const yelpAuthHeaders: string[] = [];
    ...
    const fetchFn = (async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("api.yelp.com")) {
        yelpUrls.push(url);
        yelpAuthHeaders.push((init?.headers as Record<string, string> | undefined)?.Authorization ?? "");
        return jsonResponse({
          businesses: [
            {
              id: "y1",
  ...
  it("respects category filter", async () => {
    const provider = new MoviesProvider({ tmdbApiKey: "tmdb", googleApiKey: "google" }, { fetchFn: (async () => jsonResponse({})) as typeof fetch });
