...
function makePlan(id: string, source: string, category: Plan["category"] = "movies"): Plan {
  return validatePlan({
    id,
    source,
...
class ArrayProvider implements PlanProvider {
  public calls = 0;
  public seenCategories: SearchPlansInput["categories"] | undefined;
...
class ThrowingProvider implements PlanProvider {
  public calls = 0;
...
describe("never-empty fallback logic", () => {
  it("triggers on empty base with provider errors and backfills to min", async () => {
    ...
    const curated = new ArrayProvider(
      "curated",
      Array.from({ length: 30 }, (_, i) => makePlan(`curated:${i + 1}`, "curated", i % 2 === 0 ? "movies" : "food"))
    );
