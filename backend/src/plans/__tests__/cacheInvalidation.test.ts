...
function makePlan(id: string, category: Plan["category"]): Plan {
  return {
    id,
    source: "test",
    sourceId: id,
...
function baseInput(overrides: Partial<SearchPlansInput> = {}): SearchPlansInput {
  return {
...
describe("PlanSearchCache invalidation + memory bounds", () => {
  ...
  it("invalidates by provider tag", () => {
    const shared = new MemoryCache<Plan[]>();
  ...
  it("evicts older entries when maxEntries is exceeded", () => {
    const memory = new MemoryCache<Plan[]>({ maxEntries: 2, pruneIntervalMs: 0 });
