...
export interface VideoTipDependencies {
  getVideo(videoId: string): Promise<{ id: string; canonicalPlaceId: string; authorUserId: string; authorProfileId?: string; status: string; moderationStatus: string } | undefined>;
...
export class DryadTipsService {
  ...
  private async create(input: { videoId: string; senderUserId: string; senderWalletPublicKey: string; amountAtomic: bigint; note?: string; allowSelfTip?: boolean; platformFeeBps?: number }): Promise<DryadVideoTipIntent> {
    ...
    if (!isValidSolanaPublicKey(input.senderWalletPublicKey)) throw new ValidationError(["invalid sender wallet"]);
    ...
    const tip: DryadVideoTipIntent = {
      ...
      recipientWalletPublicKey: recipientWallet.publicKey,
      grossAmountAtomic: input.amountAtomic,
      platformFeeAtomic,
      recipientNetAtomic,
    ...
    this.store.saveLedgerEvent({ id: `tle_${randomUUID()}`, tipIntentId: tip.id, status: "created", createdAt: now, payload: { amountDisplay: amountToDisplay(input.amountAtomic, 9) } });
    return clone(tip);
  }
  ...
  async submitTip(input: { tipIntentId: string; senderUserId: string }): Promise<DryadVideoTipIntent> {
    ...
    const idempotencyKey = stableIdempotencyKey([tip.id, tip.senderWalletPublicKey, tip.recipientWalletPublicKey, tip.grossAmountAtomic.toString()]);
