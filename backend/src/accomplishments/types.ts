...
export interface AccomplishmentCondition {
  metric:
    ...
    | "saved_places_count"
    | "distinct_cities_explored"
    | "helpful_reviews_count"
    | "trusted_creator_state"
    | "trusted_reviews_count"
    | "creator_streak_days"
    | "distinct_categories_reviewed";
  threshold: number;
}
...
export interface AccomplishmentDefinition {
  ...
  rarity: BadgeRarity;
  featured?: boolean;
  tiers?: Array<{ tier: number; name: string; condition: AccomplishmentCondition; xpReward?: number }>;
  condition?: AccomplishmentCondition;
  collectible?: CollectibleDefinition;
  trustGate?: { minTrustScore?: number; requireTrustedCreator?: boolean; maxModerationStrikes?: number };
}
