...
# Usage:
#   IMAGE_NAME="/full/path/to/cleric_sheet.png" ~/split_perbug_cleric_nfts.sh
#   IMAGE_NAME="/full/path/to/cleric_sheet.png" OUTPUT_DIR="$HOME/perbug-cleric-nfts" UPSCALE_SIZE=512 ~/split_perbug_cleric_nfts.sh
#
# Required:
#   IMAGE_NAME -> full path to the cleric class sheet
#
# Optional:
#   OUTPUT_DIR   -> output folder (default: $HOME/perbug-cleric-nfts)
...
OUTPUT_DIR="${OUTPUT_DIR:-$HOME/perbug-cleric-nfts}"
...
log "Splitting Perbug cleric NFT sheet from: $IMAGE_NAME"
...
COLOR_WORDS = {
    "red":    ["ember", "scarlet", "pyre", "crimson", "flame"],
    "orange": ["sunfire", "auric", "amber", "cinder", "hearth"],
    "yellow": ["radiant", "golden", "halo", "solar", "dawn"],
    "green":  ["verdant", "bloom", "moss", "vine", "spring"],
    "cyan":   ["mist", "glacier", "aqua", "frost", "tidal"],
    "blue":   ["azure", "storm", "sapphire", "deep", "calm"],
    "purple": ["violet", "arcane", "dusk", "amethyst", "void"],
    "pink":   ["rose", "glimmer", "blush", "aurora", "soft"],
    "white":  ["ivory", "pearl", "moon", "ashen", "pale"],
    "dark":   ["shadow", "night", "obsidian", "onyx", "grave"],
}
...
def analyze_sprite(im):
    ...
    bright_glow = sum(1 for (r, g, b) in pixels if max(r, g, b) > 210 and (max(r, g, b) - min(r, g, b)) > 30)
    glow_ratio = bright_glow / max(1, len(pixels))

    score = 0.0
    score += min(1.0, sat_avg) * 0.18
    score += min(1.0, uniqueish / 18.0) * 0.18
    score += min(1.0, glow_ratio * 2.5) * 0.24
    score += min(1.0, fill_ratio * 1.08) * 0.08

    if color_family in ("yellow", "white", "blue", "green", "purple"):
        score += 0.12
    if glow_ratio > 0.15:
        score += 0.10
    if sat_avg > 0.48:
...
def build_name(index, color_family):
    ...
    right = CLERIC_WORDS[(index // len(COLOR_WORDS[color_family])) % len(CLERIC_WORDS)]
    return f"{left}_{right}"

metadata_path = os.path.join(output_dir, "perbug_cleric_metadata.csv")
rows = []
...
print(f"[OK] Wrote {len(rows)} cleric NFTs into {output_dir}")
