...
# Usage:
#   IMAGE_NAME="/full/path/to/caster_sheet.png" ~/split_perbug_caster_nfts.sh
#   IMAGE_NAME="/full/path/to/caster_sheet.png" OUTPUT_DIR="$HOME/perbug-caster-nfts" UPSCALE_SIZE=512 ~/split_perbug_caster_nfts.sh
#
# Required:
#   IMAGE_NAME -> full path to the caster class sheet
#
# Optional:
#   OUTPUT_DIR   -> output folder (default: $HOME/perbug-caster-nfts)
...
OUTPUT_DIR="${OUTPUT_DIR:-$HOME/perbug-caster-nfts}"
...
log "Splitting Perbug caster NFT sheet from: $IMAGE_NAME"
...
COLOR_WORDS = {
    "red":    ["ember", "scarlet", "infernal", "pyre", "crimson"],
    "orange": ["cinder", "sunfire", "amber", "auric", "molten"],
    "yellow": ["solar", "gilded", "radiant", "spark", "dawn"],
    "green":  ["verdant", "moss", "thorn", "viridian", "bloom"],
    "cyan":   ["frost", "tidal", "mist", "glacier", "aqua"],
    "blue":   ["azure", "storm", "cobalt", "sapphire", "deep"],
    "purple": ["arcane", "violet", "void", "amethyst", "dusk"],
    "pink":   ["rose", "aurora", "blush", "prism", "glimmer"],
...
def analyze_sprite(im):
    ...
    score += min(1.0, glow_ratio * 2.6) * 0.24
    score += min(1.0, fill_ratio * 1.15) * 0.10

    if color_family in ("purple", "cyan", "blue", "pink"):
        score += 0.10
    if glow_ratio > 0.16:
        score += 0.08
    if sat_avg > 0.55:
...
def build_name(index, color_family):
    ...
    right = CASTER_WORDS[(index // len(COLOR_WORDS[color_family])) % len(CASTER_WORDS)]
    return f"{left}_{right}"

metadata_path = os.path.join(output_dir, "perbug_caster_metadata.csv")
rows = []
...
print(f"[OK] Wrote {len(rows)} caster NFTs into {output_dir}")
