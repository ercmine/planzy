...
# Usage:
#   IMAGE_NAME="/full/path/to/support_sheet.png" ~/split_perbug_support_nfts.sh
#   IMAGE_NAME="/full/path/to/support_sheet.png" OUTPUT_DIR="$HOME/perbug-support-nfts" UPSCALE_SIZE=512 ~/split_perbug_support_nfts.sh
#
# Required:
#   IMAGE_NAME -> full path to the support class sheet
#
# Optional:
#   OUTPUT_DIR   -> output folder (default: $HOME/perbug-support-nfts)
...
OUTPUT_DIR="${OUTPUT_DIR:-$HOME/perbug-support-nfts}"
...
log "Splitting Perbug support NFT sheet from: $IMAGE_NAME"
...
COLOR_WORDS = {
    "red":    ["ember", "rose", "heartfire", "crimson", "warm"],
    "orange": ["sun", "amber", "auric", "dawn", "hearth"],
    "yellow": ["golden", "solar", "radiant", "halo", "light"],
    "green":  ["verdant", "bloom", "moss", "spring", "vine"],
    "cyan":   ["mist", "tidal", "frost", "aqua", "glacier"],
    "blue":   ["azure", "storm", "sapphire", "calm", "deep"],
    "purple": ["violet", "arcane", "dusk", "amethyst", "dream"],
    "pink":   ["blush", "aurora", "glimmer", "rose", "soft"],
    "white":  ["ivory", "moon", "pearl", "ashen", "pale"],
    "dark":   ["shadow", "night", "onyx", "void", "obsidian"],
}
...
def analyze_sprite(im):
    ...
    score += min(1.0, glow_ratio * 2.8) * 0.24
    score += min(1.0, fill_ratio * 1.10) * 0.08

    if color_family in ("yellow", "green", "cyan", "pink", "white"):
        score += 0.12
    if glow_ratio > 0.18:
        score += 0.10
    if sat_avg > 0.48:
...
def build_name(index, color_family):
    ...
    right = SUPPORT_WORDS[(index // len(COLOR_WORDS[color_family])) % len(SUPPORT_WORDS)]
    return f"{left}_{right}"

metadata_path = os.path.join(output_dir, "perbug_support_metadata.csv")
rows = []
...
print(f"[OK] Wrote {len(rows)} support NFTs into {output_dir}")
