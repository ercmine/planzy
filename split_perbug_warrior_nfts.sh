...
# Usage:
#   IMAGE_NAME="/full/path/to/warrior_sheet.png" ~/split_perbug_warrior_nfts.sh
#   IMAGE_NAME="/full/path/to/warrior_sheet.png" OUTPUT_DIR="$HOME/perbug-warrior-nfts" UPSCALE_SIZE=512 ~/split_perbug_warrior_nfts.sh
#
# Required:
#   IMAGE_NAME -> full path to the warrior class sheet
#
# Optional:
#   OUTPUT_DIR   -> output folder (default: $HOME/perbug-warrior-nfts)
...
OUTPUT_DIR="${OUTPUT_DIR:-$HOME/perbug-warrior-nfts}"
...
log "Splitting Perbug warrior NFT sheet from: $IMAGE_NAME"
...
COLOR_WORDS = {
    "red":    ["crimson", "scarlet", "ember", "pyre", "blood"],
    "orange": ["cinder", "amber", "auric", "sunfire", "molten"],
    "yellow": ["golden", "solar", "gilded", "spark", "dawn"],
...
def analyze_sprite(im):
    ...
    score += min(1.0, glow_ratio * 2.2) * 0.18
    score += min(1.0, fill_ratio * 1.14) * 0.10

    if color_family in ("red", "orange", "yellow", "blue", "dark"):
        score += 0.12
    if glow_ratio > 0.15:
        score += 0.09
    if sat_avg > 0.48:
...
def build_name(index, color_family):
    ...
    right = WARRIOR_WORDS[(index // len(COLOR_WORDS[color_family])) % len(WARRIOR_WORDS)]
    return f"{left}_{right}"

metadata_path = os.path.join(output_dir, "perbug_warrior_metadata.csv")
rows = []
...
print(f"[OK] Wrote {len(rows)} warrior NFTs into {output_dir}")
